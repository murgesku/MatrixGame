...
{
	void * buf;
	if (this==NULL) buf=HeapAlloc(GetProcessHeap(),0,size); else buf=HeapAlloc(m_Heap,m_Flags,size);
	if(buf==NULL) AllocationError(size);
	return buf;
}
...
{
	void * buf;
	if(this==NULL) buf=HeapAlloc(GetProcessHeap(),HEAP_ZERO_MEMORY,size);
...
{
	if((buf=HeapReAlloc(GetProcessHeap(),0,buf,size))==NULL) ERROR_E;

	if(this==NULL) buf=HeapReAlloc(GetProcessHeap(),0,buf,size);
...
__forceinline void * CHeap::ReAllocClear(void * buf, size_t size)
{
	...
	if(this==NULL) buf=HeapReAlloc(GetProcessHeap(),HEAP_ZERO_MEMORY,buf,size);
...
__forceinline void * CHeap::AllocEx(void * buf, size_t size)
{
	if(this==NULL) {
		if(size<=0 && buf!=NULL) {
			...
			if((buf=HeapReAlloc(GetProcessHeap(),0,buf,size))==NULL) AllocationError(size);
		} else {
			if((buf=HeapAlloc(GetProcessHeap(),0,size))==NULL) AllocationError(size);
		}
	} else {
		...
		if(size<=0 && buf!=NULL) {
			...
			if((buf=HeapReAlloc(m_Heap,m_Flags,buf,size))==NULL) AllocationError(size);
		} else {
			if((buf=HeapAlloc(m_Heap,m_Flags,size))==NULL) AllocationError(size);
		}
	}
	return buf;
}
...
__forceinline void * CHeap::AllocClearEx(void * buf, size_t size)
{
	if(this==NULL) {
		if(size<=0 && buf!=NULL) {
			...
			if((buf=HeapReAlloc(GetProcessHeap(),HEAP_ZERO_MEMORY,buf,size))==NULL) AllocationError(size);
		} else {
			if((buf=HeapAlloc(GetProcessHeap(),HEAP_ZERO_MEMORY,size))==NULL) AllocationError(size);
		}
	} else {
		if(size<=0 && buf!=NULL) {
			HeapFree(m_Heap,m_Flags,buf);
			...
			if((buf=HeapReAlloc(m_Heap,m_Flags | HEAP_ZERO_MEMORY,buf,size))==NULL) AllocationError(size);
		} else {
			if((buf=HeapAlloc(m_Heap,m_Flags | HEAP_ZERO_MEMORY,size))==NULL) AllocationError(size);
		}
	}
	return buf;
}
...
{
	if(buf==NULL) return;
	if(this==NULL) HeapFree(GetProcessHeap(),0,buf);
...
#else
__forceinline BASE_API void * operator new (size_t size, Base::CHeap * heap) { return heap->Alloc(size); }
__forceinline BASE_API void operator delete (void * buf, Base::CHeap * heap) { heap->Free(buf); }
#endif
//lint +e1532
