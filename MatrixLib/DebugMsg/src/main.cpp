...
bool GlobalInit()
{
    ...
    if(Global_EventR==NULL) {
    	CloseHandle(Global_EventS); Global_EventS=NULL;
    	CloseHandle(Global_Mutex); Global_Mutex=NULL;
	    UnmapViewOfFile(Global_Buf); Global_Buf=NULL;
    	CloseHandle(Global_File); Global_File=NULL;
    	return false;
...
void GlobalClear()
{
	...
	if(Global_EventR!=NULL) { CloseHandle(Global_EventR); Global_EventR=NULL; }
	if(Global_EventS!=NULL) { CloseHandle(Global_EventS); Global_EventS=NULL; }
	if(Global_Mutex!=NULL) { CloseHandle(Global_Mutex); Global_Mutex=NULL; }
    if(Global_Buf!=NULL) { UnmapViewOfFile(Global_Buf); Global_Buf=NULL; }
	if(Global_File!=NULL) { CloseHandle(Global_File); Global_File=NULL; }
	if(Global_DMSP!=NULL) { CloseHandle(Global_DMSP); Global_DMSP=NULL; }
	Global_DMSPid=0;
...
{
	DWORD id;
	if((id=GlobalMsgProcessId())==0) return NULL;
