...
class BPCompiler {
	...
	public:
		BPCompiler(CBlockPar * bp,const wchar * text,int textlen) { m_BP=bp; m_Text=text; m_TextLen=textlen; }
		~BPCompiler() {}
		...
		bool CalcNextLine(void)
		{
			...
			if(m_Line<m_TextLen) { m_Line++; return true; }
			else return false;
		}
		void CalcSkipSpace()
		{
		...
		void FindCom()
		{
			...
			if(m_Com+1>=m_LineEnd) { m_Com=m_LineEnd; m_ComSpace=m_Com; return; }
			m_ComSpace=m_Com-1;
		...
		bool FindBlock()
		{
			...
			if(m_Block>=m_ComSpace) { m_Block=-1; return false; }
			return true;
		}
		bool FindPar()
		{
			...
			if(m_Par>=m_ComSpace) { m_Par=-1; return false; }
			return true;
		}
		int ParNameSize()
		{
