...
template <class V> __forceinline void  SBigVBOne<V>::PrepareVB(void)
{
    if (IS_VB(vb)) return;
...
template <class V> __forceinline int  SBigVBSource<V>::Select(CBigVB<V> *bvb)
{
...
template <class V> __forceinline bool  SBigVBSource<V>::IsDirty(CBigVB<V> *bvb)
{
    return FLAG(bvb->m_VB_array[one].parts[part].flags, BVB_DIRTY_PART);
}

template <class V> __forceinline bool  SBigVBSource<V>::IsMarkedNoNeed(CBigVB<V> *bvb)
{
    return FLAG(bvb->m_VB_array[one].parts[part].flags, BVB_NONEED_PART);
}

template <class V> __forceinline bool  SBigVBSource<V>::Prepare(CBigVB<V> *bvb, int stl)
{
    DTRACE();
#ifdef _DEBUG
...
template <class V> __forceinline bool  SBigVBSource<V>::Prepare(CBigVB<V> *bvb)
{
    DTRACE();
#ifdef _DEBUG
    bvb->m_Preparewas = true;
...
template <class V> __forceinline void  SBigVBSource<V>::Invalidate(CBigVB<V> *bvb)
{
    MarkNoNeed(bvb);
    if (!IsDirty(bvb))
    {
...
template <class V> __forceinline void  SBigVBSource<V>::MarkNoNeed(CBigVB<V> *bvb)
{
...
template <class V> __forceinline void CBigVB<V>::ReleaseBuffers(void)
{
...
template <class V> __forceinline void CBigVB<V>::AddSource(SBigVBSource<V> *src)
{
    int index;
...
template <class V> __forceinline void CBigVB<V>::PrepareAll(void)
{
    for (int i=0; i<m_VB_count; ++i)
    {
        for (int j=0; j<m_VB_array[i].count; ++j)
