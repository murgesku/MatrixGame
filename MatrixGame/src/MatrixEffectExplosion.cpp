...
#endif
{
    ...
    for (;intense_cnt > 0; ++i)
    {
        ...
        m_Deb[i].billboard->CBillboard::CBillboard(TRACE_PARAM_CALL pos, INTENSE_INIT_SIZE, FSRND(M_PI), 0x1F807E1B ,m_BBTextures[BBT_INTENSE].tex);
    ...
    for (;sparks_cnt > 0; ++i)
    {
        ...
        m_Deb[i].bline->CBillboardLine::CBillboardLine(TRACE_PARAM_CALL pos, pos + m_Deb[i].v * (1.0f/20.0f), EXPLOSION_SPARK_WIDTH, 0xFFFFFFFF, GetBBTexI(BBT_SPARK));
        m_Deb[i].ttl = (float)RND(m_Props->deb_min_ttl, m_Props->deb_max_ttl);
