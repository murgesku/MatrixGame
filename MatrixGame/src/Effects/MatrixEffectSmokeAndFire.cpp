...
CMatrixEffectSmoke::CMatrixEffectSmoke(const D3DXVECTOR3 &pos, float ttl, float puffttl, float spawntime, DWORD color, bool intense, float speed) :
...
void  CMatrixEffectSmoke::SpawnPuff(void)
{
    ...
    EBillboardTextureSort tex = (m_EffectType==EFFECT_FIRE)?BBT_FIRE:((m_EffectType==EFFECT_SMOKE)?BBT_SMOKE:(m_EffectType==EFFECT_FIRE_INTENSE)?BBT_FIRE_INTENSE:BBT_SMOKE_INTENSE);

    if (m_BBTextures[tex].IsIntense()) 
    {
        m_Puffs[m_PuffCnt].m_Puff.CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos,5 - FRND(2), m_Puffs[m_PuffCnt].m_PuffAngle, m_Color, m_BBTextures[tex].tex);
    } else
    {
        m_Puffs[m_PuffCnt].m_Puff.CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos,5 - FRND(2), m_Puffs[m_PuffCnt].m_PuffAngle, m_Color, &m_BBTextures[tex].bbt);
    }
...
CMatrixEffectFire::CMatrixEffectFire(const D3DXVECTOR3 &pos, float ttl, float puffttl, float spawntime, float dispfactor, bool intense, float speed) :
...
void  CMatrixEffectFire::SpawnPuff(void)
{
    ...
    if (m_BBTextures[tex].IsIntense())
    {
        m_Puffs[m_PuffCnt].m_Puff.CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos,5 - FRND(2), m_Puffs[m_PuffCnt].m_PuffAngle, 0xFFFFFFFF, m_BBTextures[tex].tex);
    } else
    {
        m_Puffs[m_PuffCnt].m_Puff.CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos,5 - FRND(2), m_Puffs[m_PuffCnt].m_PuffAngle, 0xFFFFFFFF, &m_BBTextures[tex].bbt);
    }
...
void CMatrixEffectFireStream::Draw(bool now)
{
    ...
    if (now)
    {
        if (idx == 0) m_bl1.DrawNow(g_MatrixMap->m_Camera.GetDrawNowFC()); else
        if (idx == 1) m_bl2.DrawNow(g_MatrixMap->m_Camera.GetDrawNowFC()); else
        if (idx == 2) m_bl3.DrawNow(g_MatrixMap->m_Camera.GetDrawNowFC()); else
        if (idx == 3) m_bl4.DrawNow(g_MatrixMap->m_Camera.GetDrawNowFC()); else
        if (idx == 4) m_bl5.DrawNow(g_MatrixMap->m_Camera.GetDrawNowFC());

    } else
    {
        if (idx == 0) m_bl1.AddToDrawQueue(); else
        if (idx == 1) m_bl2.AddToDrawQueue(); else
        if (idx == 2) m_bl3.AddToDrawQueue(); else
        if (idx == 3) m_bl4.AddToDrawQueue(); else
        if (idx == 4) m_bl5.AddToDrawQueue();
    }
}
...
#ifdef _DEBUG
    if (m_Frame == 8) _asm int 3
#endif

    for (int i=0; i<8;++i)
    {
        m_bl[i].CBillboardLine::CBillboardLine(TRACE_PARAM_CALL pos, pos+D3DXVECTOR3(0,0,0), 0, 0xFFFFFFFF, m_BBTextures[BBT_FLAMEFRAME0 + i].tex);
    }
}
