...
bool SynBufInit() {
    if (SynBuf_File != NULL) {
        if (WaitForSingleObject(SynBuf_DMSP, 0) == WAIT_TIMEOUT)
            return true;
        SynBufClear();
    }

    if (!GlobalMsgNewSynBuf(SynBuf_FileName, SynBuf_MutexName, SynBuf_EventName))
        return false;
...
void SynBufClear() {
    ...
    if (SynBuf_DMSP != NULL) {
        CloseHandle(SynBuf_DMSP);
        SynBuf_DMSP = NULL;
    }
    LeaveCriticalSection(&SynBuf_CS);
}

bool SynBuf_Write(void *buf1, int size1, void *buf2 = NULL, int size2 = 0, void *buf3 = NULL, int size3 = 0,
                  void *buf4 = NULL, int size4 = 0, void *buf5 = NULL, int size5 = 0) {
    int i = 0;
    ...
    while (i <= 1) {
        ...
        if (buf2 != NULL)
            CopyMemory(SynBuf_Buf + cursize + 16 + size1, buf2, size2);
        if (buf3 != NULL)
            CopyMemory(SynBuf_Buf + cursize + 16 + size1 + size2, buf3, size3);
        if (buf4 != NULL)
            CopyMemory(SynBuf_Buf + cursize + 16 + size1 + size2 + size3, buf4, size4);
        if (buf5 != NULL)
            CopyMemory(SynBuf_Buf + cursize + 16 + size1 + size2 + size3 + size4, buf5, size5);
...
DEBUGMSG_API void DMcc(char *path, char *text) {
    ...
    SynBuf_Write(&zag, sizeof(SZagMsg), &lenpath, 4, path, lenpath, &lentext, 4, text, lentext);
}
...
DEBUGMSG_API void DMww(wchar_t *path, wchar_t *text) {
    ...
    SynBuf_Write(&zag, sizeof(SZagMsg), &lenpath, 4, path, lenpath * 2, &lentext, 4, text, lentext * 2);
}
