...
{
    if(td->FastBuf_File!=NULL) return true;
...
{
	if(td->FastBuf_Event!=NULL) { SetEvent(td->FastBuf_Event); CloseHandle(td->FastBuf_Event); td->FastBuf_Event=NULL; }
    if(td->FastBuf_Buf!=NULL) { UnmapViewOfFile(td->FastBuf_Buf); td->FastBuf_Buf=NULL; }
	if(td->FastBuf_File!=NULL) { CloseHandle(td->FastBuf_File); td->FastBuf_File=NULL; }
}
				  ...
				  )
{
	...
	while(i<=1) {
		...
		if(buf2!=NULL) CopyMemory(td->FastBuf_Buf+cursize+16+size1,buf2,size2);
		if(buf3!=NULL) CopyMemory(td->FastBuf_Buf+cursize+16+size1+size2,buf3,size3);
		if(buf4!=NULL) CopyMemory(td->FastBuf_Buf+cursize+16+size1+size2+size3,buf4,size4);
		if(buf5!=NULL) CopyMemory(td->FastBuf_Buf+cursize+16+size1+size2+size3+size4,buf5,size5);
