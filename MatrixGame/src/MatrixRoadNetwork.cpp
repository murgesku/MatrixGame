...
CMatrixRoad::~CMatrixRoad()
{
    ...
    DeleteZone();
    DeletePath();
    if(m_Start!=NULL) m_Start->DeleteRoad(this);
    if(m_End!=NULL) m_End->DeleteRoad(this);
    m_Start=NULL;
    m_End=NULL;
    m_Parent=NULL;
}
...
void CMatrixRoad::DeleteCrotch(const CMatrixCrotch * crotch)
{
    ...
    if(m_Start==crotch) m_Start=NULL;
    if(m_End==crotch) m_End=NULL;
}
...
void CMatrixRoad::DeletePath()
{
    ...
    if(m_Path!=NULL) { HFree(m_Path,m_Parent->m_Heap); m_Path=NULL; }
    m_PathCnt=0;
}
...
//        if(m_Path[m_PathCnt-1].Dist2(road->m_Path[0])<=m_Path[0].Dist2(road->m_Path[road->m_PathCnt-1])) { // After
...
void CMatrixRoad::CorrectStartEndByPath()
{
    ...
    if(m_Start->m_Center.Dist2(m_Path[0])>m_End->m_Center.Dist2(m_Path[0])) {
        CMatrixCrotch * t=m_Start; m_Start=m_End; m_End=t;
    }
}
...
void CMatrixRoad::DeleteZone()
{
    ...
    if(m_Zone!=NULL) { HFree(m_Zone,m_Parent->m_Heap); m_Zone=NULL; }
    m_ZoneCnt=0;
    m_ZoneCntMax=0;
}
...
CMatrixCrotch::~CMatrixCrotch()
{
    ...
    for(int i=m_RoadCnt-1;i>=0;i--) {
        if(m_Road[i]!=NULL) m_Road[i]->DeleteCrotch(this);
        m_Road[i]=NULL;
    }
    m_RoadCnt=0;
    m_Parent=NULL;
...
void CMatrixRoadRoute::Clear()
{
    ...
    m_ListCnt=0;
    m_ListCntMax=0;
    if(m_Header!=NULL) { HFree(m_Header,m_Parent->m_Heap); m_Header=NULL; }
    if(m_Units!=NULL) { HFree(m_Units,m_Parent->m_Heap); m_Units=NULL; }
}
...
void CMatrixRoadRoute::NeedEmpty(int cnt, bool strong)
{
    ...
    m_Units=(SMatrixRoadRouteUnit *)HAllocClearEx(m_Units,m_ListCntMax*m_Parent->m_CrotchCnt*sizeof(SMatrixRoadRouteUnit),m_Parent->m_Heap);
}
...
void CMatrixRoadRoute::CopyUnit(int listfrom,int listto)
{
    ...
    CopyMemory(m_Units+listto*m_Parent->m_CrotchCnt,m_Units+listfrom*m_Parent->m_CrotchCnt,m_Header[listto].m_Cnt*sizeof(SMatrixRoadRouteUnit));
}
...
void CMatrixRoadNetwork::Clear()
{
    ...
    if(m_RoadFindIndex) { HFree(m_RoadFindIndex,m_Heap); m_RoadFindIndex=NULL; }
    if(m_CrotchFindIndex) { HFree(m_CrotchFindIndex,m_Heap); m_CrotchFindIndex=NULL; }
    if(m_RegionFindIndex) { HFree(m_RegionFindIndex,m_Heap); m_RegionFindIndex=NULL; }
...
//  DWORD colors[colorscnt]={0x8000ff00,0x8000ff40,0x8000ff80,0x8000ffC0,0x8000c000,0x8000c040,0x8000c080,0x8000c0C0,0x80008000,0x80008040,0x80008080,0x800080C0};
    ...
    if(m_ZoneCnt>0) {
        ...
        for(i=0;i<m_ZoneCnt;i++) {
            if(m_Zone[i].m_Color<0 || m_Zone[i].m_Color>colorscnt) ERROR_E;
...
if(test && !g_TestLocal) {
    ...
    CHelper::Create(0,101)->Cone(v2,D3DXVECTOR3(v2.x,v2.y,v2.z+10.0f+level*2.0f),5.0f,5.0f,0xff00ff00,0xff00ff00,3);
}
#endif
            ...
            if(crotch2->m_Region==region) { crotchcur=crotch2; break; }
        }
            if(crotchcur) break;
...
if(test && !g_TestLocal) {
    ...
    v2.z=g_MatrixMap->GetZ(v2.x,v2.y);
    CHelper::Create(0,101)->Cone(v2,D3DXVECTOR3(v2.x,v2.y,v2.z+8.0f+level*2.0f),6.0f,6.0f,0xffffff00,0xffffff00,3);
}
#endif
    ...
    while(crotchcur!=cstart) {
        ...
        for(i=0;i<crotchcur->m_RoadCnt;i++) {
            ...
            if(crotch->m_Data && crotch->m_Data<level) { crotch2=crotch; road2=crotchcur->m_Road[i]; break; }
        }
        if(!crotch2) ERROR_E;
...
CHelper::Create(0,101)->Sphere(v2,5.0f,5,0xff00ff00);
        ...
        FindPathFromCrotchToRegion(mm,rr->m_Units[ln*m_CrotchCnt+rr->m_Header[ln].m_Cnt-1].m_Crotch,rlist[i],rr,test);
        if(rr->m_Header==NULL) return;
    }
}
...
void CMatrixRoadNetwork::ClearPlace()
{
    m_PlaceCnt=0;
    m_PlaceEmpty=-1;
    if(m_Place!=NULL) { HFree(m_Place,m_Heap); m_Place=NULL; }
}
...
{
    if(m_Region!=NULL) { HFree(m_Region,m_Heap); m_Region=NULL; }
    m_RegionCnt=0;
}
    ...
    for(i=0;i<rlcnt-1;i++) {
        for(u=i+1;u<rlcnt;u++) {
            if(region->m_Center.Dist2(GetRegion(rl[i])->m_Center)>region->m_Center.Dist2(GetRegion(rl[u])->m_Center)) {
                int temp=rl[i]; rl[i]=rl[u]; rl[u]=temp;
            }
        }
    }
...
int CMatrixRoadNetwork::FindPathInRegionRun(byte mm,int rstart,int rend,int * path,int maxpath,bool err)
{
    ...
    while(true) {
		...
		for(int i=0;i<r->m_NearCnt;i++) {
			...
			if(nr==rend) { nr_=nr; wtp_=wtp; break; }
			else if(wtp<=wtp_) { nr_=nr; wtp_=wtp; }
        }
		if(nr_<0) {
			ERROR_E;
		}
...
{
    if(m_PLList!=NULL) { HFree(m_PLList,m_Heap); m_PLList=NULL; }
    m_PLSizeX=0;
    m_PLSizeY=0;
}
...
void CMatrixRoadNetwork::InitPL(int mapsizex,int mapsizey)
{
    ...
    if(m_PlaceCnt<=0 || m_PLSizeX<=0 || m_PLSizeX<=0) return;
...
{
    return
    CRect(  max(0,mapcoords.left >> m_PLShift),
            max(0,mapcoords.top >> m_PLShift),
    ...
    for(int y=plr.top;y<plr.bottom;y++,plist+=m_PLSizeX-(plr.right-plr.left)) {
        for(int x=plr.left;x<plr.right;x++,plist++) {
            ...
            for(int u=0;u<plist->m_Cnt;u++,place++) {
                if(mappos.x>=place->m_Pos.x && mappos.y>=place->m_Pos.y && mappos.x<(place->m_Pos.x+4) && mappos.y<(place->m_Pos.y+4)) {
