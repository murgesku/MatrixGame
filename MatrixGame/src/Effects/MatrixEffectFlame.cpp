...
m_Owner(owner), m_Pos(pos), m_Dir(dir), m_Speed(speed), m_Next(NULL), m_Prev(NULL), m_Shleif(NULL), m_CurAlpha(0), m_Break(0)
#ifdef _DEBUG
...
//    }
    ...
    for (int i = 0; i<FLAME_NUM_BILLS; ++i)
    {
        ...
        {
            m_Flames[i].m_Flame.CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos,1,0,0xFFFFFFFF,m_Owner->m_BBTextures[BBT_FLAME].tex);
        } else
        {
            m_Flames[i].m_Flame.CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos,1,0,0xFFFFFFFF,&m_Owner->m_BBTextures[BBT_FLAME].bbt);
        }
...
void CFlamePuff::Takt(float step)
{
    ...
    } else
    {
        if (m_Prev && (!m_Break))
        {
            ...
            m_Shleif->CBillboardLine::CBillboardLine(TRACE_PARAM_CALL m_Prev->m_Pos, m_Pos, m_Scale * 2.0f, (a << 24) | 0xFFFFFF, m_Owner->GetBBTexI(BBT_FLAMELINE));
        }
    }
