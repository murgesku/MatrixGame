...
CMatrixEffect(), m_Pos(pos), m_Radius(r), m_InitRadius(r)
{
    ...
    for (int i = 0; i<m_SelCnt; ++i)
    {
        ...
        for (int j = 0; j<SEL_BLUR_CNT; ++j)
        {
            ...
            {
                m_Points[i].m_Blur[j].CBillboard::CBillboard(TRACE_PARAM_CALL cpos, SEL_SIZE * sz, 0, color, m_BBTextures[BBT_SELDOT].tex);
            } else
            {
                m_Points[i].m_Blur[j].CBillboard::CBillboard(TRACE_PARAM_CALL cpos, SEL_SIZE * sz, 0, color, &m_BBTextures[BBT_SELDOT].bbt);
            }
