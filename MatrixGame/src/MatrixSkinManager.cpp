...
const SSkin * CSkinManager::GetSkin(const wchar *textures, DWORD gsp)
{
    ...
    for (int i=0;i<n;++i)
    {
        ...
        if (i==2) sk.m_TexBack = (CTextureManaged *)g_Cache->Get(cc_TextureManaged, temp.Get()); else
        if (i==3) sk.m_TexMask = (CTextureManaged *)g_Cache->Get(cc_TextureManaged, temp.Get()); else
        if (i==4)
        {
...
void CSkinManager::Clear(void)
{
    for (int gsp = 0; gsp<GSP_COUNT; ++gsp)
    {
        ...
        if (m_Skins[gsp]) HFree(m_Skins[gsp], g_MatrixHeap);
    }
