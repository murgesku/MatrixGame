...
#if D3DX_SDK_VERSION >= 21
            m_Font->DrawTextW(NULL, m_Items[i].key->Get(), m_Items[i].key->GetLen(), &r, DT_NOCLIP|DT_LEFT|DT_VCENTER|DT_SINGLELINE, color);
#else
...
#if D3DX_SDK_VERSION >= 21
            m_Font->DrawTextW(NULL,m_Items[i].val->Get(), m_Items[i].val->GetLen(), &r, DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOCLIP, color);
#else
