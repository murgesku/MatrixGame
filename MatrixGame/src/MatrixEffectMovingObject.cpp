...
CMatrixEffectMovingObject::~CMatrixEffectMovingObject()
{
    ...
    if (m_Props.handler == MO_Bomb_Takt)
    {
...
{
#ifdef _DEBUG
    if (m_Props.object == NULL) _asm int 3
#endif
...
static bool HMEnum(const D3DXVECTOR3 &fpos, CMatrixMapStatic *ms, DWORD user)
{
    ...
    if (hmd->props->common.hm.target != NULL && ((hmd->props->common.hm.target->m_Type == OBJECT_TYPE_FLYER) || (hmd->props->common.hm.target->m_Type == OBJECT_TYPE_ROBOTAI) || (hmd->props->common.hm.target->m_Type == OBJECT_TYPE_CANNON)) && (ms->GetObjectType() == OBJECT_TYPE_BUILDING) )
    {
        return true; // no need building if robot located
    }
    
    selectnew =    (hmd->props->common.hm.target != NULL ) && (hmd->props->common.hm.target->m_Type == OBJECT_TYPE_BUILDING) && 
    ...
    if ( (cc > 0.98480775301220806f) && ((cc > hmd->maxcos) || selectnew || (oc == hmd->props->common.hm.target)))
    {
        ...
        CMatrixMapStatic *t = g_MatrixMap->Trace(NULL, hmd->props->curpos, *p + (*p - hmd->props->curpos) * 0.1f, TRACE_ALL, hmd->props->skip);
        if ((t == TRACE_STOP_NONE) || (t == ms))
        {
            if (hmd->props->common.hm.target != NULL) hmd->props->common.hm.target->Release();
...
void MO_Homing_Missile_Takt(D3DXMATRIX &m, SMOProps &props, float takt)
{
    ...
    D3DXVECTOR2 vmin(seekcenter.x-HOMING_RADIUS,seekcenter.y-HOMING_RADIUS), vmax(seekcenter.x+HOMING_RADIUS,seekcenter.y+HOMING_RADIUS);
    ...
    g_MatrixMap->FindObjects(seekcenter, HOMING_RADIUS, 1, TRACE_BUILDING|TRACE_ROBOT|TRACE_CANNON|TRACE_FLYER, props.skip, HMEnum, (DWORD)&data);

    if (data.found == false)
    {
        if (props.common.hm.target) props.common.hm.target->Release();
...
skip_obj_seek:
        ...
        if (props.time > props.common.hm.next_fire_time)
        {
            ...
            while (props.time > props.common.hm.next_fire_time)
            {
                ...
                if (props.time < 200)
                {
                    ...
                    {
                        ((CMatrixEffectShleif *)props.shleif->effect)->
                        AddSmoke(p, 300, 300, 400, 0xFFFFFFFF, true, 0);
                    }
                } else
                {
                    ...
                    {
                        ((CMatrixEffectShleif *)props.shleif->effect)->
                            AddSmoke(p, 300, 600, 800, 0xFFFFFFFF, true, 0);

                        ((CMatrixEffectShleif *)props.shleif->effect)->
                            AddFire(p, 300, 300, 800, 1, true, 0);
                    }
                }
    ...
    if (hit || distance < MISSILE_IMPACT_RADIUS_SQ) 
    {
        ...
        hit |= g_MatrixMap->FindObjects(props.curpos, MISSILE_IMPACT_RADIUS, 1, props.hitmask, props.skip, MOEnum, (DWORD)&props);
        ...
        if (hit)
        {
            ...
            {
                CMatrixEffect::CreateLandscapeSpot(NULL, D3DXVECTOR2(hitpos.x, hitpos.y), FSRND(M_PI), FRND(3) + 6, SPOT_VORONKA);
...
void MO_Gun_Takt(D3DXMATRIX &m, SMOProps &props, float takt)
{
    ...
    {
        CMatrixEffect::CreateBillboardLine(NULL, props.curpos, hitpos, 6, 0x8FFFFFFF, 0, 1000, CMatrixEffect::GetBBTexI(BBT_SHLEIF) );
...
void MO_Gun_cannon_Takt(D3DXMATRIX &m, SMOProps &props, float takt)
{
    ...
    if (props.common.gun.in_frustum_count > 0) 
    {
        CMatrixEffect::CreateBillboardLine(NULL, props.curpos, hitpos, 4, 0x6FFFFFFF, 0, 500, CMatrixEffect::GetBBTexI(BBT_SHLEIF) );
        //DM("cc", CStr(props.curpos.x)  + ","+CStr(props.curpos.y) + ","+ CStr(props.curpos.z) + ","+ CStr(hitpos.x) + ","+ CStr(hitpos.x) + ","+  CStr(hitpos.x));
    }
...
void CMatrixEffectBuoy::Takt(float step)
{
    ...
    if ( m_Props.common.buoy.angle > M_PI_MUL(2) ) m_Props.common.buoy.angle -= M_PI_MUL(2);
    D3DXMatrixRotationYawPitchRoll(&mr, m_Props.startpos.x, m_Props.startpos.y, m_Props.startpos.z + m_Props.common.buoy.angle);
    ...
    if (IRND(7000) < step)
    {
        ...
        CMatrixEffect::CreateBillboard(NULL, m_Props.curpos, 1, 30, 0xFF000000 | m_BuoyColor, m_BuoyColor, 1000,0 ,TEXTURE_PATH_WAVE,D3DXVECTOR3(1,0,0));
        CMatrixEffect::CreateBillboard(NULL, m_Props.curpos, 1, 30, 0xFF000000 | m_BuoyColor, m_BuoyColor, 1000,300 ,TEXTURE_PATH_WAVE,D3DXVECTOR3(1,0,0));
        CMatrixEffect::CreateBillboard(NULL, m_Props.curpos, 1, 30, 0xFF000000 | m_BuoyColor, m_BuoyColor, 1000,600 ,TEXTURE_PATH_WAVE,D3DXVECTOR3(1,0,0));
    }
    //if (IRND(7000) < takt)
    {
