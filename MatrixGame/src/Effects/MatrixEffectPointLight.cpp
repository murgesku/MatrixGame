...
CMatrixEffect(), m_Pos(pos), m_Radius(r), m_Color(color), m_Bill(0), m_Time(0), m_PointLum(m_Heap)
{
    ...
    if (drawbill)
    {
        ...
        {
            m_Bill->CBillboard::CBillboard(TRACE_PARAM_CALL pos,r*POINTLIGHT_BILLSCALE,0,color, m_BBTextures[BBT_POINTLIGHT].tex);
        } else
        {
            m_Bill->CBillboard::CBillboard(TRACE_PARAM_CALL pos,r*POINTLIGHT_BILLSCALE,0,color, &m_BBTextures[BBT_POINTLIGHT].bbt);
        }
    }
...
#define ADDVERT(v) if (v->index < 0) { v->index = cidx++;  verts->p = v->p; verts->tu0 = v->tu0;  verts->tv0 = v->tv0; \
                /*verts->color = m_Color;*/ \
                verts->color = 0; \
                verts->tu1 = v->tu1; verts->tv1 = 0.5f; \
                if (v->mp != NULL)  { \
                    light.lum = v->lum; \
                    light.mp = v->mp; \
                    m_PointLum.Any<SMapPointLight>(light); }  ++verts; ++m_NumVerts; } else { if (v->index < midx) midx = v->index; \
