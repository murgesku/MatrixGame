...
public:
    ...
    CBillboard(TRACE_PARAM_DEF const D3DXVECTOR3 &pos, float scale, float angle, DWORD color, const SBillboardTexture *tex); // sorted
    CBillboard(TRACE_PARAM_DEF const D3DXVECTOR3 &pos, float scale, float angle, DWORD color, CTextureManaged *tex);         // intense
    ~CBillboard()
    {
        DTRACE();
        // do nothing!
...
#ifdef _DEBUG
        if (m_FirstIntense != NULL) {  __asm int 3  }
        release_called = true;
#endif
...
public:
    ...
    CBillboardLine(TRACE_PARAM_DEF const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1, float width, DWORD color, CTextureManaged *tex);
    ~CBillboardLine()
    {
        // do nothing
#ifdef _DEBUG
