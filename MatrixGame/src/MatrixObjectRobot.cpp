...
void CMatrixRobot::UnitInsert(int beforeunit, ERobotUnitType type, ERobotUnitKind kind)
{
	...
	if(beforeunit<m_UnitCnt) MoveMemory(m_Unit+beforeunit+1,m_Unit+beforeunit,(m_UnitCnt-beforeunit)*sizeof(SMatrixRobotUnit));
	ZeroMemory(m_Unit+beforeunit,sizeof(SMatrixRobotUnit));

	m_UnitCnt++;

	m_Unit[beforeunit].m_Type=type;
	m_Unit[beforeunit].m_Kind=kind;
	D3DXMatrixIdentity(&m_Unit[beforeunit].m_Matrix);


    m_Unit[beforeunit].m_NextAnimTime = (float)g_MatrixMap->GetTime();
...
void CMatrixRobot::WeaponInsert(int beforeunit, ERobotUnitType type, ERobotUnitKind kind, int hullno, int pilon)
{
    ...
    m_Unit[beforeunit].m_LinkMatrix=g_MatrixMap->m_RobotWeaponMatrix[hullno-1].list[fis_pilon-1].id;
    m_Unit[beforeunit].m_Invert=(g_MatrixMap->m_RobotWeaponMatrix[hullno-1].list[fis_pilon-1].access_invert & SETBIT(31)) != 0;
...
void CMatrixRobot::UnitDelete(int nounit)
{
    ...
    if (m_CurrState != ROBOT_DIP && m_Unit[nounit].m_Kind == RUK_CHASSIS_ANTIGRAVITY && m_Unit[nounit].m_Type == MRT_CHASSIS)
    {
    ...
    {
        if (m_Unit[nounit].m_WeaponRepairData)
        {
            m_Unit[nounit].m_WeaponRepairData->Release();
            m_Unit[nounit].m_WeaponRepairData = NULL;
        }
	    if(m_Unit[nounit].m_ShadowStencil)
        {
            HDelete(CVOShadowStencil,m_Unit[nounit].m_ShadowStencil,g_MatrixHeap);
            m_Unit[nounit].m_ShadowStencil=NULL;
        }
    }

	if(nounit<(m_UnitCnt-1)) memcpy(m_Unit+nounit,m_Unit+nounit+1,(m_UnitCnt-1-nounit)*sizeof(SMatrixRobotUnit));
	m_UnitCnt--;
...
void CMatrixRobot::UnitClear(void)
{
	...
	while(m_UnitCnt) UnitDelete(m_UnitCnt-1);
...
void CMatrixRobot::WeaponSelectMatrix(void)
{
    ...
    {
        if(m_Unit[i].m_Type==MRT_ARMOR) break;
    }
    if(i>=m_UnitCnt) return;

    int hullno=m_Unit[i].m_Kind-1;
    bool maccess[MR_MAXUNIT]; for(i=0;i<MR_MAXUNIT;i++) maccess[i]=true;
    for(i=0;i<m_UnitCnt;i++)
    {
        if(m_Unit[i].m_Type!=MRT_WEAPON) continue;
        int t;
        for(t=0;t<g_MatrixMap->m_RobotWeaponMatrix[hullno].cnt;t++)
        {
            if(!maccess[t]) continue;
            if(!(g_MatrixMap->m_RobotWeaponMatrix[hullno].list[t].access_invert & (1<<(m_Unit[i].m_Kind-1)))) continue;
            maccess[t]=false;
            break;
        }
        if(t>=g_MatrixMap->m_RobotWeaponMatrix[hullno].cnt) ERROR_E;
        m_Unit[i].m_LinkMatrix=g_MatrixMap->m_RobotWeaponMatrix[hullno].list[t].id;
        m_Unit[i].m_Invert=(g_MatrixMap->m_RobotWeaponMatrix[hullno].list[t].access_invert & SETBIT(31)) != 0;
    }
}
...
float CMatrixRobot::Z_From_Pos(void)
{
    ...
    if(roboz < WATER_LEVEL)
    {
        ...
        if(m_Unit[0].m_Kind == RUK_CHASSIS_HOVERCRAFT || m_Unit[0].m_Kind == RUK_CHASSIS_ANTIGRAVITY) roboz = WATER_LEVEL;
...
DCP();
            ...
            if(TruncFloat(m_PosX * INVERT(GLOBAL_SCALE)) >= (g_MatrixMap->m_Size.x) || TruncFloat(m_PosY * INVERT(GLOBAL_SCALE)) >= g_MatrixMap->m_Size.y || m_PosX < 0 || m_PosY < 0)
            {
#ifdef _DEBUG
                _asm int 3
#endif
            ...
            else
            {
                ...
                //(вероятно, это следствие, а не причина, т.к. код видит, что робот не находится в нужной точке на базу уже после того, как его телепортировало)
...
DCP();
            ...
            m_Core->m_Matrix._21 = m_Forward.x;    m_Core->m_Matrix._22 = m_Forward.y;    m_Core->m_Matrix._23 = m_Forward.z;        m_Core->m_Matrix._24 = 0;
...
DCP();
            ...
            {
                robot_z = g_MatrixMap->GetZ(m_PosX,m_PosY);
            }
            else
            {
                robot_z = u->m_Base->GetFloorZ();
            }
...
DCP();
            ...
            m_Unit[0].m_IMatrix = m_Core->m_IMatrix;
        }
...
DCP();
            if(m_Unit[i].m_Type==MRT_WEAPON)
            {
                ...
                if(!www && m_Unit[i].m_Kind == RUK_WEAPON_REPAIR)
                {
                    www = true;
                }
            ...
            else if(m_Unit[i].m_Type == MRT_ARMOR)
            {
                ...
                if(g_MatrixMap->GetPlayerSide()->GetArcadedObject() == this && m_Unit[0].m_Kind == RUK_CHASSIS_PNEUMATIC)
                {
...
calc:
        ...
        {
            //D3DXMATRIX mx;
            //BuildRotateMatrix(mx, *(D3DXVECTOR3 *)&m_Unit[narmor].m_Matrix._41, *(D3DXVECTOR3 *)&m_Matrix._11, -0.4f);
            //m_Unit[0].m_Matrix *= mx;
            //D3DXMatrixInverse(&m_Unit[0].m_IMatrix, NULL, &m_Unit[0].m_Matrix);
            ...
            m_ChassisData.m_LStream->SetPos(spos, spos + sdir * m_ChassisData.m_StreamLen);
            ...
            D3DXVec3TransformCoord(&spos, (D3DXVECTOR3 *)&m->_41, &m_Unit[0].m_Matrix);
            D3DXVec3TransformNormal(&sdir, &sdir, &m_Unit[0].m_Matrix);
            m_ChassisData.m_RStream->SetPos(spos, spos + sdir * m_ChassisData.m_StreamLen);
        ...
        if(www)
        {
            if(IsInterfaceDraw())
            {
                ...
                for(int i = 0; i < m_UnitCnt; ++i)
                {
                    if(m_Unit[i].m_Type == MRT_WEAPON && m_Unit[i].m_Kind == RUK_WEAPON_REPAIR)
                    {
                        if(m_Unit[i].m_WeaponRepairData == NULL)
                        {
                            m_Unit[i].m_WeaponRepairData = SWeaponRepairData::Allocate();
                        }

                        m_Unit[i].m_WeaponRepairData->Update(m_Unit + i);
                    }
                }
	...
	if(need & m_RChange & MR_ShadowStencil)
    {
		...
		if(m_ShadowType!=SHADOW_STENCIL)
        {
			for(int i=0; i<m_UnitCnt; ++i)
            {
                ...
                {
                    HDelete(CVOShadowStencil,m_Unit[i].m_ShadowStencil,g_MatrixHeap); m_Unit[i].m_ShadowStencil=NULL; 
                }
			}
		}
        else
...
//					if(!(m_Unit[i].m_Graph->VO()->EdgeExist())) m_Unit[i].m_Graph->VO()->EdgeBuild();
                ...
                m_Unit[i].m_ShadowStencil->Build(*(m_Unit[i].m_Graph->VO()),m_Unit[i].m_Graph->GetVOFrame(), light,len, m_Unit[i].m_Invert != 0);

                if(!m_Unit[i].m_ShadowStencil->IsReady())
                {
	...
	if(need & m_RChange & MR_ShadowProjGeom)
    {
        ...
        else 
        {
            ...
            ShadowProjBuildGeomList(*m_ShadowProj,m_UnitCnt,obj,noframe,wm,m_Core->m_Matrix, m_Core->m_IMatrix,g_MatrixMap->m_LightMain,int(100/GLOBAL_SCALE),false);
...
void CMatrixRobot::DoAnimation(int cms)
{
    ...
    {
        if (
            (m_Unit[0].m_Kind == RUK_CHASSIS_HOVERCRAFT) ||
            (m_Unit[0].m_Kind == RUK_CHASSIS_PNEUMATIC) ||
            (m_Unit[0].m_Kind == RUK_CHASSIS_ANTIGRAVITY)
           )
        {
	        for(int i=0;i<1;i++)
            {
...
void CMatrixRobot::Takt(int cms)
{
    ...
    if (m_CurrState != ROBOT_CARRYING && m_CurrState != ROBOT_FALLING && m_CurrState != ROBOT_IN_SPAWN && m_CurrState != ROBOT_BASE_CAPTURE)
    {
        if (FLAG(m_ObjectState,ROBOT_FLAG_ONWATER))
        {
            m_KeelWaterCount += m_Speed * KEELWATER_SPAWN_FACTOR * cms;
        } else if (m_Unit[0].m_Kind == RUK_CHASSIS_HOVERCRAFT)
        {
        ...
        {
            CMatrixEffect::CreateBillboard(NULL, D3DXVECTOR3(m_PosX, m_PosY, WATER_LEVEL), 5, 40, 0xFFFFFFFF, 0x00FFFFFF, 3000,0 ,TEXTURE_PATH_KEELWATER,m_Forward);
    ...
    if(m_Unit[0].m_Kind != RUK_CHASSIS_PNEUMATIC) DoAnimation(cms); // proceed animation only for NON pneumatic chassis
...
bool CMatrixRobot::PickFull(const D3DXVECTOR3 &orig, const D3DXVECTOR3 &dir, float *outt)  const
{
	...
	for(int i = 0; i < m_UnitCnt; ++i)
    {
		if(m_Unit[i].m_Graph)
        {
            if(m_Unit[i].m_Graph->PickFull(m_Unit[i].m_Matrix, m_Unit[i].m_IMatrix, orig, dir, outt)) return true;
		}
	}
	return false;
}
    ...
    */
    ...
    if(g_Config.m_ShowStencilShadows && !IsInterfaceDraw())
    {
	    for(int i = 0; i < m_UnitCnt; ++i)
        {
            ...
            if(m_CurrState != ROBOT_DIP && m_Unit[i].m_ShadowStencil) m_Unit[i].m_ShadowStencil->BeforeRender();
	    }
    }
    ...
    if(m_Unit[0].m_Kind == RUK_CHASSIS_ANTIGRAVITY)
    {
        m_ChassisData.m_LStream->BeforeDraw();
    }
}
...
//        // draw foots
//
//        if (m_Unit[0].m_Kind == RUK_CHASSIS_PNEUMATIC && m_Animation == ANIMATION_MOVE)
//        {
//
    ...
    if(!FLAG(g_MatrixMap->m_Flags, MMFLAG_OBJECTS_DRAWN))
    {
        if(IsInterfaceDraw())
        {
	        for(int i = 0; i < m_UnitCnt; ++i)
            {
                if(m_Unit[i].m_Type == MRT_WEAPON && m_Unit[i].m_Kind == RUK_WEAPON_REPAIR)
                {
                    if(m_Unit[i].m_WeaponRepairData)
                    {
                        // draw
                        m_Unit[i].m_WeaponRepairData->Draw(true);
                    }
                }
            }
        ...
        {   
            if(IsInterfaceDraw())
            {
                ...
                m_ChassisData.m_LStream->Draw(true);
...
void CMatrixRobot::FirstLinkPneumatic(void)
{
    ...
    D3DXVec2TransformCoord(&m_ChassisData.m_LinkPos, &m_Pneumaic[vof+vof_add].foot, &m);
...
{
    if (m_Pneumaic) HFree(m_Pneumaic, g_MatrixHeap);
}
