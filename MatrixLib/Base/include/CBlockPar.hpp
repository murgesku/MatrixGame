...
class BASE_API CBlockPar : public CMain {
	...
	public:
		...
		CWStr ParGetNE(const CWStr & name, int index = 0)	const										{ const CWStr * str=ParGetNE_(name.Get(),name.GetLen(),index); if(str!=NULL) return *str; else return CWStr(); }
		CWStr ParGetNE(const wchar * name, int index = 0)	const										{ const CWStr * str=ParGetNE_(name,WStrLen(name),index); if(str!=NULL) return *str; else return CWStr(); }
