...
bool IntersectTriangle(const D3DXVECTOR3 & orig, const D3DXVECTOR3 & dir, const D3DXVECTOR3 & v0, const D3DXVECTOR3 & v1, const D3DXVECTOR3 &v2, float & ot, float & ou, float & ov)
{
    ...
    if((uu<0.0f) || (uu > det)) return false;

	D3DXVec3Cross(&qvec,&tvec,&edge1);

	float vv=D3DXVec3Dot(&dir,&qvec);
    if((vv<0.0f) || ((uu + vv)>det)) return false;
...
bool IntersectLine(const D3DXVECTOR2 & s1,const D3DXVECTOR2 & e1,const D3DXVECTOR2 & s2,const D3DXVECTOR2 & e2, D3DXVECTOR2 & out)
{
    float l1x=e1.x-s1.x;
    float l1y=e1.y-s1.y;
    float l2x=e2.x-s2.x;
    ...
    if(fabs(kofdel)<0.0001f) return false;

    out.x=((s2.y-s1.y)*l1x*l2x+l1y*l2x*s1.x-l2y*l1x*s2.x)/kofdel;

    if(l1x!=0) out.y=((out.x-s1.x)*l1y)/l1x+s1.y;
    else out.y=((out.x-s2.x)*l2y)/l2x+s2.y;

    return true;
}

bool IntersectLine(const D3DXVECTOR2 & s1,const D3DXVECTOR2 & e1,const D3DXVECTOR2 & s2,const D3DXVECTOR2 & e2, float * t, float * u)
{
	D3DXVECTOR2 b=e1-s1;
	D3DXVECTOR2 d=e2-s2;
	D3DXVECTOR2 c=s2-s1;
	...
	if(fabs(dtb)<0.0001f) return false;

	if(t) *t=(-d.y*c.x+d.x*c.y)/dtb;
	if(u) *u=(-b.y*c.x+b.x*c.y)/dtb;
	return true;
}

bool IsIntersectRect(float sx1,float sy1,float ex1,float ey1,float sx2,float sy2,float ex2,float ey2,float * sx,float * sy)
{
	...
	*sy=0.0f;
	...
	if(fabs(dfc_x)<mr_x && fabs(dfc_y)<mr_y) {
		if(fabs(dfc_x)<mr_x) {
			if(dfc_x<0) *sx=dfc_x+mr_x;
			else *sx=dfc_x-mr_x;
		}
		if(fabs(dfc_y)<mr_y) {
			if(dfc_y<0) *sy=dfc_y+mr_y;
			else *sy=dfc_y-mr_y;
		}
		return true;
	} else return false;
...
{
	float r2 = r*r;
	if((D3DXVec2LengthSq(&(center - s)) < r2) && (D3DXVec2LengthSq(&(center - e)) < r2))
		return false;
...
void CTrajectory::Continue1(const D3DXVECTOR3 *points, int pcnt)
{
    ...
    for (int i = 1; i<=pcnt; ++i)
    {
        ...
        m_Segments[i+m_SegCnt].Build1((i0>=0)?points[i0]:p[4+i0], (i1>=0)?points[i1]:p[4+i1], points[i2], points[i3]);
    }
...
void CTrajectory::Continue2(const D3DXVECTOR3 *points, int pcnt)
{
    ...
    for (int i = 1; i<=pcnt; ++i)
    {
        ...
        m_Segments[i+m_SegCnt].Build2((i0>=0)?points[i0]:p[4+i0], (i1>=0)?points[i1]:p[4+i1], points[i2], points[i3]);
    }
