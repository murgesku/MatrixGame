...
{
    if(m_Target!=skip && m_Target && m_Target->GetObjectType()==OBJECT_TYPE_BUILDING) m_Target=NULL;
    if(m_TargetAttack!=skip && m_TargetAttack && m_TargetAttack->GetObjectType()==OBJECT_TYPE_BUILDING) m_TargetAttack=NULL;
...
void CInfo::RemoveAllSlowely()
{
    ...
    while(enemie){
        ...
        enemie = enemie->m_NextEnemy;
        if(e2->m_DelSlowly) {
            if(e2->GetEnemy()==m_Target) m_Target=NULL;
            if(e2->GetEnemy()==m_TargetAttack) m_TargetAttack=NULL;
...
{
    if(m_Target == ms) m_Target = NULL;
    if(m_TargetAttack == ms) m_TargetAttack = NULL;
