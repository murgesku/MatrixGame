...
int ff_GetInt(const wchar * tstr,int len)
{
	...
	for(int i=0;i<len;i++) {
		ch=tstr[i];
		if(ch>='0' && ch<='9') zn=zn*10+ch-'0';
	}
	for(int i=0;i<len;i++) if(tstr[i]=='-') { zn=-zn; break; }
...
int ff_Int64ToStr(__int64 zn,wchar * str,int base)
{
	...
	if(zn==0) {
		*str=L'0';
		return 1;
	} else if(zn<0) { si=true; zn=-zn; }
	
	while(zn!=0) {
		int nom=(int)(zn % base);
		if(nom<10) *str=nom + L'0'; else *str=nom-10 + L'A';
		zn=zn/base;
		count++;
		str--;
	}
...
void ff_FormatInt64(CWStr & cs,__int64 zn,wchar * str,int lenstr,int width,wchar fill,int align,int base)
{
	...
	if(width!=0 && tlen>width) tt=width;
	else tt=tlen;
...
int ff_DoubleToStr(double zn,wchar * str,int zpz)
{
	...
	if(sign!=0) { *str=L'-'; str++; outcount++; }

	le=strlen(st);
	if(dec<0) {
		*str=L'0'; str++; outcount++;
		if(le>0) {
			*str=L'.'; str++; outcount++;
			for(int i=0;i<-dec;i++) { *str=L'0'; str++; outcount++; }
			for(int i=le-1;i>=0;i--) if(st[i]=='0') count++; else break;
			if(le>count) {
				for(int yu=0;yu<le-count;yu++) { *str=wchar(st[yu]); str++; outcount++; }
			}
		}
	} else {
		if(dec>0) {
			for(int yu=0;yu<dec;yu++) { *str=wchar(st[yu]); str++; outcount++; }
		} else { *str=L'0'; str++; outcount++; }

		for(int i=le-1;i>=dec;i--) if(st[i]=='0') count++; else break;
		if(dec<le-count) {
			*str=L'.'; str++; outcount++;
			for(int yu=0;yu<le-count-dec;yu++) { *str=wchar(st[dec+yu]); str++; outcount++; }
		}
	}
	return outcount;
}
