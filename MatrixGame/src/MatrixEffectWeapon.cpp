...
void CMatrixEffectWeapon::WeaponHit(CMatrixMapStatic *hiti, const D3DXVECTOR3 &pos, DWORD user, DWORD flags)
{
    ...
    if (hiti == TRACE_STOP_WATER && w->m_Type != WEAPON_FLAMETHROWER && w->m_Type != WEAPON_BIGBOOM)
    {
    ...
    case WEAPON_VOLCANO:
        {
            ...
            } else
            {
                ...
                CMatrixEffect::CreateKonus(NULL, hitpos, splash, 5, 5, FSRND(M_PI), 300, true, (CTextureManaged *)g_Cache->Get(cc_TextureManaged,TEXTURE_PATH_GUN_BULLETS2));
    ...
    case WEAPON_CANNON1:
        {
            ...
            int f = IRND(3);
            if (f == 0)
                CMatrixEffect::CreateBillboardLine(NULL, m_Pos, m_Pos + m_Dir*50 , 30, 0xFFFFFFFF, 0, 1000, m_BBTextures[BBT_GUNFIRE1].tex);
            else if (f == 1)
                CMatrixEffect::CreateBillboardLine(NULL, m_Pos, m_Pos + m_Dir*50 , 30, 0xFFFFFFFF, 0, 1000, m_BBTextures[BBT_GUNFIRE2].tex);
            else 
                CMatrixEffect::CreateBillboardLine(NULL, m_Pos, m_Pos + m_Dir*50 , 30, 0xFFFFFFFF, 0, 1000, m_BBTextures[BBT_GUNFIRE3].tex);
    ...
    case WEAPON_LIGHTENING:
        {
            ...
            if (s == TRACE_STOP_LANDSCAPE)
            {
                CMatrixEffect::CreateLandscapeSpot(NULL, D3DXVECTOR2(hitpos.x, hitpos.y), FSRND(M_PI), FRND(1) + 0.5f, SPOT_PLASMA_HIT);
    ...
    case WEAPON_CANNON2:
        {
            ...
            } else
            {
                ...
                if (!g_MatrixMap->AddEffect(m_Laser)) m_Laser = NULL;
            }
    ...
    case WEAPON_BIGBOOM:
        {
            ...
            m_Ref++;
            CMatrixEffect::CreateBigBoom(m_Pos, m_WeaponDist * m_WeaponCoefficient, 300, TRACE_ALL, NULL/*m_Skip*/,(DWORD)this,WeaponHit);
    ...
    case WEAPON_REPAIR:
        {
            ...
            {
                m_Repair = (CMatrixEffectRepair *)CreateRepair(m_Pos, m_Dir, m_WeaponDist * m_WeaponCoefficient, m_Skip);
                if (!g_MatrixMap->AddEffect(m_Laser)) m_Repair = NULL;
            }
            ...
            if (m_Repair)
            {
                ...
                {
                    if (m_Handler) m_Handler(TRACE_STOP_NONE, m_Pos, m_User, FEHF_LASTHIT);
    ...
    case WEAPON_CANNON0:
        {
            ...
            int f = IRND(3);
            if (f == 0)
                CMatrixEffect::CreateBillboardLine(NULL, m_Pos, m_Pos + m_Dir*35 , 25, 0xFFFFFFFF, 0, 1000, m_BBTextures[BBT_GUNFIRE1].tex);
            else if (f == 1)
                CMatrixEffect::CreateBillboardLine(NULL, m_Pos, m_Pos + m_Dir*35 , 25, 0xFFFFFFFF, 0, 1000, m_BBTextures[BBT_GUNFIRE2].tex);
            else 
                CMatrixEffect::CreateBillboardLine(NULL, m_Pos, m_Pos + m_Dir*35 , 25, 0xFFFFFFFF, 0, 1000, m_BBTextures[BBT_GUNFIRE3].tex);
...
CMatrixEffect(), m_bl(TRACE_PARAM_CALL pos0, pos1, LASER_WIDTH, 0xFFFFFFFF, m_BBTextures[BBT_LASER].tex)
{
    ...
    {
        m_end.CBillboard::CBillboard(TRACE_PARAM_CALL pos0,LASER_WIDTH*0.5f, 0, 0xFFFFFFFF, m_BBTextures[BBT_LASEREND].tex);
    } else
    {
        m_end.CBillboard::CBillboard(TRACE_PARAM_CALL pos0,LASER_WIDTH*0.5f, 0, 0xFFFFFFFF, &m_BBTextures[BBT_LASEREND].bbt);
    }
}
...
m_bl1(TRACE_PARAM_CALL start, start+dir*VOLCANO_FIRE_LENGHT, VOLCANO_FIRE_WIDTH, 0xFFFFFFFF, m_BBTextures[BBT_GUNFIRE1].tex),
m_bl2(TRACE_PARAM_CALL start, start+dir*VOLCANO_FIRE_LENGHT, VOLCANO_FIRE_WIDTH, 0xFFFFFFFF, m_BBTextures[BBT_GUNFIRE2].tex),
m_bl3(TRACE_PARAM_CALL start, start+dir*VOLCANO_FIRE_LENGHT, VOLCANO_FIRE_WIDTH, 0xFFFFFFFF, m_BBTextures[BBT_GUNFIRE3].tex)
{
...
void CVolcano::Draw(void)
{
    ...
    if (idx == 0) m_bl1.AddToDrawQueue(); else
    if (idx == 1) m_bl2.AddToDrawQueue(); else
    if (idx == 2) m_bl3.AddToDrawQueue();
}