...
public:
    ...
    void        DX_Prepare(void) {m_preVB.Prepare(m_VB); m_preIB.Prepare(m_IB);};
    void        DX_Free(void) {m_preVB.MarkNoNeed(m_VB); m_preIB.MarkNoNeed(m_IB);}
    ...
    void Render(void)
    {
        DTRACE();
#ifdef _DEBUG
        if (!m_Tex) ERROR_S(L"Oops, texture is NULL in shadow rendering.");
#else
        if (!m_Tex) return; // just return
#endif
