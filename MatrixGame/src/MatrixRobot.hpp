...
enum OrderType
{
	...
	ROT_MOVE_TO,
	ROT_MOVE_TO_BACK,
    ROT_MOVE_RETURN,          // –обот пропускает другого робота затем возвращаетс€ в эту позицию
    ROT_STOP_MOVE,
	ROT_FIRE,
	ROT_STOP_FIRE,
    ROT_CAPTURE_FACTORY,
...
class CMatrixRobotAI : public CMatrixRobot {
    ...
    int                 m_Cols;
    int                 m_Team;
    int                 m_GroupLogic;                       // dab. ¬ какой лагической группе находитс€ робот
    int                 m_Group;
    int                 m_OrdersInPool;
	...
	int                 m_MapX,m_MapY;                      //
	int                 m_ZoneCur;				            // «она в которой находитс€ робот
	int                 m_DesX,m_DesY;		                // “очка в которую хочет встать робот
	int                 m_ZoneDes;				            // «она в которую стремитс€ робот
	int                 m_ZonePathNext;			            // —ледующа€ зона в пути 
	int                 m_ZonePathCnt;			            //  оличество зон (включительно начальную и конечную) до m_ZoneDes
	int                 m_ZoneNear;				            // Ѕлижайша€ зона от той в которой находитс€ робот по напровлению к m_ZoneDes
	int                 m_MovePathCnt;			            //  оличество точек в пути движени€
	int                 m_MovePathCur;			            // “екуща€ точка в пути движени€
    int*                m_ZonePath;			                // —писок зон до m_ZoneDes
    CPoint              m_MovePath[MatrixPathMoveMax];      // ѕуть движени€
    float               m_MovePathDist;                     // ƒлина расчитанного пути
    float               m_MovePathDistFollow;               // —колько робот прошол по пути
    D3DXVECTOR2         m_MoveTestPos;
    int                 m_MoveTestChange;

    float               m_Strength;                         // —ила робота

    int                 m_NextTimeAblaze;
    int                 m_NextTimeShorted;
    D3DXVECTOR3         m_CollAvoid;

    float               m_SpeedWaterCorr;                   // коррекци€ скорости при движении по воде
    float               m_SpeedSlopeCorrDown;               // коррекци€ скорости при движении с горы
    float               m_SpeedSlopeCorrUp;                 // коррекци€ скорости при движении в гору

	float               m_maxSpeed;		                    // максимально развиваема€ скорость
    float               m_maxHullSpeed;                     // скорость поворота HULL'a
    float               m_maxRotationSpeed;                 // скорость вращени€ робота на месте
    float               m_MaxFireDist;
    float               m_MinFireDist;
    float               m_RepairDist;
    float               m_SyncMul;
...
//    int                 m_GatherPeriod;
    CInfo               m_Environment;
    int                 m_HaveRepair;                       // 0-нет 1-есть чинилка 2-все оружи€ это чинилка (не учитываетс€ бомба)
...
public:
    ...
    float       GetStrength(void)                                   { return m_Strength; } // —ила робота
    void        CalcStrength(void);                                 // –асчитываем силу робота
    ...
    void ZoneCurFind(void);		                    // Ќайти зону в которой находитс€ робот (Out: m_ZoneCur)
	void ZonePathCalc(void);	                    // –ассчитать путь до m_ZoneDes (In: m_ZoneCur,m_ZoneDes) (Out: m_ZonePathCnt,m_ZonePath)
	void ZoneMoveCalc(void);                        // –ассчитать путь движени€ до ближайшей зоны (In: m_ZoneCur,m_ZoneNear) (Out: m_MovePathCnt,m_MovePathCur,m_MovePath)
    float CalcPathLength(void);
    //void ZoneMoveCalcTo(void);	                // –ассчитать путь в нутрии текущей зоны до точки назначени€ (In: m_DesX,m_DesY) (Out: m_MovePathCnt,m_MovePathCur,m_MovePath)
	void MoveByMovePath(int ms);	                // ƒвигатьс€ по пути движени€
    void MoveToRndBuilding();


    void CalcRobotMass();                           // вычисл€ет массу, скорость, силу робота
    bool Seek(const D3DXVECTOR3 &dest,bool & rotate,bool end_path, bool back=false);// поиск вектора смещени€ робота
    ...
    void RobotWeaponInit();                         // инициализаци€ оружи€
