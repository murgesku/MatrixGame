...
bool CMatrixMapObject::Damage(EWeapon weap, const D3DXVECTOR3 &, const D3DXVECTOR3 &, int attacker_side, CMatrixMapStatic* attaker)
{
    DTRACE();

    if (attacker_side != PLAYER_SIDE && FLAG(m_ObjectState, OBJECT_STATE_SPECIAL)) return false; // only player can hit this objects
    ...
    if (m_BehFlag == BEHF_BURN)
    {
        ...
        if (weap == WEAPON_BIGBOOM || weap == WEAPON_HOMING_MISSILE || weap == WEAPON_BOMB || weap == WEAPON_PLASMA || weap == WEAPON_FLAMETHROWER)
        {
            if (!IsAblaze())
            {
                MarkAblaze();
            ...
            {
                if (GetAblazeTTL() > 1500) SetAblazeTTL(1500);
            } else
            {
                if (GetAblazeTTL() > 15000) SetAblazeTTL(15000);
            }
            AddLT();
        }
    } else if (m_BehFlag == BEHF_TERRON && !FLAG(m_ObjectState, OBJECT_STATE_TERRON_EXPL))
    {
        ...
        if (!m_Graph->IsAnim(L"Pain"))
        {
            ...
            int idx = IRND(4);
            if (idx == 0) CSound::AddSound(S_TERRON_PAIN1, GetGeoCenter());
            else if (idx == 1) CSound::AddSound(S_TERRON_PAIN2, GetGeoCenter());
            else if (idx == 2) CSound::AddSound(S_TERRON_PAIN3, GetGeoCenter());
            else if (idx == 3) CSound::AddSound(S_TERRON_PAIN4, GetGeoCenter());
        }
...
void CMatrixMapObject::ClearTexture(CBaseTexture *tex)
{
    ...
    {
        if ( m_ShadowTextures[i].tex == (CTextureManaged *)tex ) m_ShadowTextures[i].tex = NULL;
        if ( m_ShadowTextures[i].tex_burn == (CTextureManaged *)tex ) m_ShadowTextures[i].tex_burn = NULL;
    }
}
...
void CMatrixMapObject::BeforeDraw(void)
{
    ...
    if (m_BehFlag == BEHF_BREAK && FLAG(m_ObjectState, OBJECT_STATE_SPECIAL) && m_PB > 0 && m_BreakHitPoint > 0)
    {
        ...
        if (dist < 1000)
        {
            ...
            {
                D3DXVECTOR2 p = g_MatrixMap->m_Camera.Project(pos, g_MatrixMap->GetIdentityMatrix());
                m_PB->Modify(p.x - (PB_SPECIAL_WIDTH*0.5f), p.y - GetRadius(), float(m_BreakHitPoint) / float(m_BreakHitPointMax));
            }
        }
    }
...
void CMatrixMapObject::Draw(void)
{
    ...
    if (FLAG(m_ObjectState, OBJECT_STATE_NORMALIZENORMALS)) { g_D3DD->SetRenderState( D3DRS_NORMALIZENORMALS,  TRUE ); }
    ...
    if (FLAG(m_ObjectState, OBJECT_STATE_NORMALIZENORMALS)) g_D3DD->SetRenderState( D3DRS_NORMALIZENORMALS,  FALSE );
...
void CMatrixMapObject::LogicTakt(int ms)
{
    ...
    } else if (m_BehFlag == BEHF_SPAWNER)
    {
        ...
        } else if (m_PrevStateRobotsInRadius == 0)
        {
            if (g_MatrixMap->GetTime() > GetAblazeTTL())
            {
                int addt = 107;
                bool found = false;
                g_MatrixMap->FindObjects(*(D3DXVECTOR3 *)&m_Core->m_Matrix._41, m_SensRadius, 1, TRACE_ROBOT, NULL, FindOnlyPlayerRobots, (DWORD)&found);
                if (found)
                {
                    CWStr temp(g_MatrixMap->IdsGet(m_Type).GetStrPar(OTP_BEHAVIOUR, L"*"), g_CacheHeap);
                    m_PrevStateRobotsInRadius = 1;
        ...
        } else if (m_PrevStateRobotsInRadius == 2)
        {
            if (m_Graph->IsAnimEnd() && m_SpawnRobotCore)
            {
                if (m_SpawnRobotCore->m_Object)
                {
                    ...
                    g_MatrixMap->FindObjects(*(D3DXVECTOR3 *)&m_Core->m_Matrix._41, m_SensRadius, 1, TRACE_ROBOT, NULL, FindOnlyPlayerRobotsTgt, (DWORD)&tgt);
    ...
    } else if (m_BehFlag == BEHF_SENS)
    {
        ...
        if (GetAblazeTTL() < 0)
        {
            ...
            if (g_MatrixMap->FindObjects(*(D3DXVECTOR3 *)&m_Core->m_Matrix._41, m_SensRadius, 1, TRACE_ROBOT, NULL, NULL, 0))
            {
                if (m_PrevStateRobotsInRadius == 0)
                {
                    CWStr temp(g_MatrixMap->IdsGet(m_Type).GetStrPar(OTP_BEHAVIOUR, L"*"), g_CacheHeap);
        ...
        while (g_MatrixMap->GetTime() > m_NextTime)
        {
            ...
            if (IRND(10000) < GetAblazeTTL())
            {
                ...
                if (cnt > 0)
                {
                    ...
                    {
                        CMatrixEffect::CreateFireAnim(NULL, pos + dir * (t + 6), 20, 30 + FRND(20), GetAblazeTTL()-Float2Int(FRND(4000)));
                        CMatrixEffect::CreateSmoke(NULL, pos + dir * (t + 6) + D3DXVECTOR3(0,0,10), 100, 1300, 20, 0x80303030, false, 1.0f / 30.0f);
                    } else
                    {
                        CMatrixEffect::CreateSmoke(NULL, pos + dir * (t + 2), 100, 1300, 20, 0x40303030, false, 1.0f / 30.0f);

                    }
                    //CMatrixEffect::CreateFire(NULL, pos + dir * (t + 2), 100, 2000, 10, 2, false);
                }

                if (Damage(WEAPON_ABLAZE, pos, dir,0,NULL)) return; // it is no mater who ablaze this object
            }
        }
