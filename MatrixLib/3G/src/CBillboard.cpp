    ...
    if (m_FirstIntense || CBillboardLine::m_First)
    {
        ...
        while (cur)
        {
            ...
            if (cur->m_Next == cur->m_NextTex)
            {
                ...
                if (++groupscnt >= MAX_BB_TEXGROUPS) break;
...
{
    if (m_VB != NULL) DESTROY_VB(m_VB);
    if (m_IB != NULL) DESTROY_IB(m_IB);

}

CBillboard::CBillboard(TRACE_PARAM_DEF const D3DXVECTOR3 &pos, float scale, float angle, DWORD color, const SBillboardTexture *tex):
#ifdef _DEBUG
...
CBillboard::CBillboard(TRACE_PARAM_DEF const D3DXVECTOR3 &pos, float scale, float angle, DWORD color, CTextureManaged *tex):         // intense
#ifdef _DEBUG
m_file(_file), m_line(_line),
#endif
...
CBillboardLine::CBillboardLine(TRACE_PARAM_DEF const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1, float width, DWORD color, CTextureManaged *tex):
#ifdef _DEBUG
m_file(_file), m_line(_line),
#endif
