...
void CTerSurface::ClearSurfaces(void)
{
    ...
    if (m_Surfaces) HFree(m_Surfaces, g_MatrixHeap);
...
{
    if (m_BigVB == NULL) m_BigVB = CBigVB<STerSurfVertex>::NewBigVB(g_MatrixHeap);
    ...
    if (m_BigIB == NULL) m_BigIB = CBigIB::NewBigIB(g_MatrixHeap);
...
{
    if (m_BigVBM == NULL) m_BigVBM = CBigVB<STerSurfVertexM>::NewBigVB(g_MatrixHeap);
