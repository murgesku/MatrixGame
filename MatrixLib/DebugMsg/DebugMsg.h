...
DEBUGMSG_API void __cdecl DM(char *path, char *text);
DEBUGMSG_API void __cdecl DM_(char *path, char *text);
DEBUGMSG_API void __cdecl DM(wchar_t *path, wchar_t *text);
DEBUGMSG_API void __cdecl DM_(wchar_t *path, wchar_t *text);

#ifndef DMDEFINES
#define DMDEFINES
// hack!
inline void DM(const char *s1, const char *s2) {
    DM((char *)s1, (char *)s2);
};
inline void DM_(const char *s1, const char *s2) {
    DM_((char *)s1, (char *)s2);
}
inline void DM(const wchar_t *s1, const wchar_t *s2) {
    DM((wchar_t *)s1, (wchar_t *)s2);
}
inline void DM_(const wchar_t *s1, const wchar_t *s2) {
    DM_((wchar_t *)s1, (wchar_t *)s2);
}
#endif