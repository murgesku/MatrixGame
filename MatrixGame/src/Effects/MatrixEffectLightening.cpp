...
CMatrixEffectLightening::CMatrixEffectLightening(const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1, float ttl, float dispers, float width, DWORD color, bool bp):
CMatrixEffect(), m_Pos0(pos0),m_Pos1(pos1), m_TTL(ttl), m_Color(color), m_End0(NULL),
m_Dispersion(dispers), m_Width(width)
{
    DTRACE();
    ...
    if (bp)
    {
        ...
        {
            m_End0->CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos0, width/2, 0, color, m_BBTextures[BBT_GLOWBEAMEND].tex);
        } else
        {
            m_End0->CBillboard::CBillboard(TRACE_PARAM_CALL m_Pos0, width/2, 0, color, &m_BBTextures[BBT_GLOWBEAMEND].bbt);
        }
    }
...
void CMatrixEffectLightening::SetPos(const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1)
{
    ...
    } else
    {
        ...
        while (m_BL_cnt < cnt)
        {
            m_BL[m_BL_cnt].CBillboardLine::CBillboardLine(TRACE_PARAM_CALL pos0, pos1, m_Width, m_Color, GetBBTexI(BBT_GLOWBEAM));
            ++m_BL_cnt;
        }
...
void CMatrixEffectShorted::SetPos(const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1)
{
    ...
    } else
    {
        ...
        while (m_BL_cnt < cnt)
        {
            m_BL[m_BL_cnt].CBillboardLine::CBillboardLine(TRACE_PARAM_CALL pos0, pos1, SHORTED_WIDTH, m_Color, GetBBTexI(BBT_GLOWBEAM));
            ++m_BL_cnt;
        }
