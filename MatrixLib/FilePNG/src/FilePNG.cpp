...
DWORD FilePNG_ReadStart_Buf(void * soubuf,DWORD soubuflen,DWORD * lenx,DWORD * leny,DWORD * countcolor,DWORD * format)
{
	...
	try {
		...
		if((data->png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING,
												   ...
												   FilePNG_png_default_warning)) == NULL) throw "Error";

		if((data->info_ptr = png_create_info_struct(data->png_ptr)) == NULL) throw "Error";
...
DWORD FilePNG_Read(DWORD id,void * buf,DWORD lenline,DWORD * arraycolor)
{
	...
	try {
		...
		for(i=0,tbuf=(BYTE *)buf;i<data->LenY;i++,tbuf+=lenline) row_pointers[i]=tbuf;
		...
		if(data->png_ptr!=NULL) png_destroy_read_struct(&(data->png_ptr),&(data->info_ptr), (png_infopp)NULL);
		if(data->fFree!=NULL && data->SouBuf!=NULL) free(data->SouBuf);
		delete data;
		return 0;
	}
...
int FilePNG_Write(void * bufout,int bufoutlen,void * buf,DWORD ll,DWORD lx,DWORD ly,DWORD bytepp,int rgb_to_bgr)
{
	...
	try {
		png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING,NULL,FilePNG_png_default_error, FilePNG_png_default_warning);
		if (png_ptr == NULL) throw "Error";

		info_ptr = png_create_info_struct(png_ptr);
		if (info_ptr == NULL) throw "Error";
		...
		for (i = 0; i < int(ly); i++) rows[i] = ((BYTE *)buf) + i*ll;
