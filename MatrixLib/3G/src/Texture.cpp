...
autoload:
	...
	if (FAILED(D3DXCreateTextureFromFileInMemoryEx(g_D3DD,buf.Get(),buf.Len(),0,0,FLAG(m_Flags,TF_NOMIPMAP)?1:0,0,D3DFMT_UNKNOWN,pool,D3DX_FILTER_NONE,D3DX_FILTER_BOX,0,NULL,NULL,&ret)))
    {
        return NULL;
    }
    return ret;
...
void CTexture::OnResetDevice(void)
{
    ...
    if (!FLAG(m_Flags, TF_LOST) || m_Tex == NULL) return; // not lost
...
void CTexture::LoadFromBitmap(CBitmap & bm)
{
    ...
    FAILED_DX(D3DXCreateTextureFromFileInMemoryEx(g_D3DD,buf.Get(),buf.Len(),0,0,FLAG(m_Flags,TF_NOMIPMAP)?1:0,0,D3DFMT_UNKNOWN,D3DPOOL_DEFAULT,D3DX_FILTER_TRIANGLE | D3DX_FILTER_DITHER,D3DX_FILTER_BOX,0,NULL,NULL,&m_Tex));
