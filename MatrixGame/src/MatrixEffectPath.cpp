...
CMatrixEffect(),  m_Kill(false), m_Points(m_Heap), m_Dirs(m_Heap), m_Lens(m_Heap), m_Cnt(cnt), m_Takt(0), m_NextTakt(0), m_Dots(NULL),
m_DotsCnt(0), m_First(NULL), m_Last(NULL), m_Angle(0), m_Barier(PATH_HIDE_DISTANCE)
...
void CMatrixEffectPath::Takt(float step)
{
    ...
    if (dtime > 0.9f) dtime = 0.9f;

    m_Angle += 0.05f * dtime;
    if (m_Angle > (2*M_PI)) m_Angle -= float(2*M_PI);
