...
void CMatrixMapLogic::Clear()
{
    ...
    if(m_MapPoint)
    {
        HFree(m_MapPoint, g_MatrixHeap); m_MapPoint = NULL;
    }
...
void CMatrixMapLogic::Takt(int step)
{
    ...
    if(IsPaused()) 
    {
	    ...
	    while(ms)
        {
            ...
            else if(ms->IsBuildingAlive()) ms->AsBuilding()->PauseTakt(step);
            else if(ms->IsCannonAlive()) ms->AsCannon()->PauseTakt(step);
            else if(ms->IsRobotAlive()) ms->AsRobot()->PauseTakt(step);
            ms = ms->GetNextLogic();
	    }
    ...
    //    for(int i = 0; i<m_EffectsCnt; ++i) m_Effects[i]->Takt(nt);
...
//Нигде не используется и не делает ничего
/*
