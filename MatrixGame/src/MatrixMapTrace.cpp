...
CMatrixMapStatic * CMatrixMap::Trace(D3DXVECTOR3 *result, const D3DXVECTOR3 &start, const D3DXVECTOR3 &end, DWORD mask, CMatrixMapStatic *skip)
{
    DTRACE();
    ...
    {
        if (data.dz == 0) return TRACE_STOP_NONE;
...
//  trace landscape
    ...
    scan_y_complete: ;
        ...
        if (scanhit == SR_BREAK)
        {
            if (data.obj != NULL) goto obj_found;
            return TRACE_STOP_NONE;
        }
        if (scanhit == SR_FOUND)
        {
