...
CVOShadowStencil::~CVOShadowStencil()
{
    ...
    if (m_Frames) HFree(m_Frames, m_Heap);
    DX_Free();
...
void CVOShadowStencil::Build(CVectorObject & obj, int frame, const D3DXVECTOR3 & vLight, float len, bool invert)
{
    ...
    if (obj.m_Geometry.m_EdgesCnt == 0) return;
    ...
    if (dirty || m_FrameFor != frame)
    {
        ...
        if (!dirty && vlight.all == fd->m_light.all && fabs(len-fd->m_len) < 1.0) return;

    } else
    {
         // frame match
        if (vlight.all == fd->m_light.all && fabs(len-fd->m_len) < 1.0) return;
    }
    fd->m_len = len;
...
#define MUL_Z(v) (vlight.z*v)
	...
	for(;keb < kee; ++keb)
    {
        ...
        int vi0 = keb->v00 / sizeof(SVOVertex) - frr->m_EdgeVertexIndexMin, vi1 = keb->v01 / sizeof(SVOVertex) - frr->m_EdgeVertexIndexMin;
#ifdef _DEBUG
        if (vi0 >= frr->m_EdgeVertexIndexCount) _asm int 3
        if (vi1 >= frr->m_EdgeVertexIndexCount) _asm int 3
#endif
        if (verts[vi0] < 0)
        {
            verts[vi0] = verts_c;
