...
public:
    ...
    //Вынесены в паблик, чтобы была возможность проверять их состояние
    ...
    int Inc()
    {
        if(++m_Counter > UP_LIMIT) m_Counter = UP_LIMIT;
        return m_Counter;
    }
    int Dec()
    {
        if(--m_Counter < DOWN_LIMIT) m_Counter = DOWN_LIMIT;
        return m_Counter;
    }
    ...
    void Disable()
    {
        if(m_ButtonDown) m_ButtonDown->SetState(IFACE_DISABLED);
        if(m_ButtonUp) m_ButtonUp->SetState(IFACE_DISABLED);
        m_Counter = 0;
    }
    void Enable()
    {
        if(m_ButtonDown) m_ButtonDown->SetState(IFACE_NORMAL);
        if(m_ButtonUp) m_ButtonUp->SetState(IFACE_NORMAL);
        Reset();
        CheckUp();
    }
