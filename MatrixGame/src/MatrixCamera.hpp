...
public:
    ...
    void ZoomOutStep(void) {
        ...
        if (m_DistParam[m_ModeIndex] > 4.0f)
            m_DistParam[m_ModeIndex] = 4.0f;
        if (m_DistParam[m_ModeIndex] < 0.25f)
            m_DistParam[m_ModeIndex] = 0.25f;
    ...
    void SetDrawNowParams(const D3DXMATRIX &iview, const D3DXVECTOR3 &fc) {
        m_DN_iview = iview;
        m_DN_fc = fc;
    }  // hack function
    ...
    __forceinline D3DXVECTOR3 ProjectNorm(const D3DXVECTOR3 &pos) {
        ...
        out.z = GetViewMatrix()._13 * pos.x + GetViewMatrix()._23 * pos.y + GetViewMatrix()._33 * pos.z +
                GetViewMatrix()._43;
        return out;
    }
...
__forceinline bool CMatrixCamera::IsInFrustum(const D3DXVECTOR3 &p) const {
    ...
    return false; if ( (tp.x * GetFrustPlaneR().norm.x + tp.y * GetFrustPlaneR().norm.y + tp.z *
    GetFrustPlaneR().norm.z) < 0.0f) return false; if ( (tp.x * GetFrustPlaneT().norm.x + tp.y * GetFrustPlaneT().norm.y
    ...
    */
    ...
    if (!GetFrustPlaneR().IsOnSide(p))
        return false;
    if (!GetFrustPlaneT().IsOnSide(p))
        return false;
    if (!GetFrustPlaneB().IsOnSide(p))
        return false;
    return true;
}
...
__forceinline bool CMatrixCamera::IsInFrustum(const D3DXVECTOR3 &p, float r) const {
    ...
    if (!GetFrustPlaneR().IsOnSide(p + GetFrustPlaneR().norm * r))
        return false;
    if (!GetFrustPlaneT().IsOnSide(p + GetFrustPlaneT().norm * r))
        return false;
    if (!GetFrustPlaneB().IsOnSide(p + GetFrustPlaneB().norm * r))
        return false;
    return true;
}
...
__forceinline bool CMatrixCamera::IsInFrustum(const D3DXVECTOR3 &mins, const D3DXVECTOR3 &maxs) const {
    ...
    if (GetFrustPlaneR().BoxSide(mins, maxs) == 2)
        return false;
    if (GetFrustPlaneB().BoxSide(mins, maxs) == 2)
        return false;
    if (GetFrustPlaneT().BoxSide(mins, maxs) == 2)
        return false;
    return true;
}
