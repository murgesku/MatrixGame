...
bool CIFaceButton::OnMouseRBDown()
{
    ...
    if(ElementCatch(mouse) /*&& ElementAlpha(mouse)*/)
    {
        if(m_strName == IF_BASE_PILON1)
        {
            if(g_PopupMenu)
            {
                ...
                for(int i = 1; i < MENU_WEAPONNORM_ITEMS; i++)
                {
                    ...
                    if(!player_side->m_ConstructPanel->IsEnoughResourcesForThisPieceOfShit(0, MRT_WEAPON, ERobotUnitKind(kind)))
                    {
        ...
        else if(m_strName == IF_BASE_PILON2)
        {
            if(g_PopupMenu)
            {
                for(int i = 1; i < MENU_WEAPONNORM_ITEMS; i++)
                {
                    ...
                    if(!player_side->m_ConstructPanel->IsEnoughResourcesForThisPieceOfShit(1, MRT_WEAPON, ERobotUnitKind(kind)))
                    {
                        g_PopupWeaponNormal[i].color = NERES_LABELS_COLOR;
                    }
                    else
        ...
        else if(m_strName == IF_BASE_PILON3)
        {
            if(g_PopupMenu)
            {
                for(int i = 1; i < MENU_WEAPONNORM_ITEMS; i++)
                {
                    ...
                    if(!player_side->m_ConstructPanel->IsEnoughResourcesForThisPieceOfShit(2, MRT_WEAPON, ERobotUnitKind(kind)))
                    {
                        g_PopupWeaponNormal[i].color = NERES_LABELS_COLOR;
                    }
                    else
        ...
        else if(m_strName == IF_BASE_PILON4)
        {
            if(g_PopupMenu)
            {
                for(int i = 1; i < MENU_WEAPONNORM_ITEMS; i++)
                {
                    ...
                    if(!player_side->m_ConstructPanel->IsEnoughResourcesForThisPieceOfShit(3, MRT_WEAPON, ERobotUnitKind(kind)))
                    {
                        g_PopupWeaponNormal[i].color = NERES_LABELS_COLOR;
                    }
                    else
        ...
        else if(m_strName == IF_BASE_PILON5)
        {
            if(g_PopupMenu)
            {
                if(!player_side->m_ConstructPanel->IsEnoughResourcesForThisPieceOfShit(4, MRT_WEAPON, ERobotUnitKind(RUK_WEAPON_MORTAR)))
                {
                ...
                if(!player_side->m_ConstructPanel->IsEnoughResourcesForThisPieceOfShit(4, MRT_WEAPON, ERobotUnitKind(RUK_WEAPON_BOMB))){
