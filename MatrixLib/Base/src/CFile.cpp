...
void CFile::Open(DWORD shareMode)
{
	if(m_Open==0) 
    {
        if (IS_UNICODE())
        {
		    m_Handle = CreateFileW((const wchar *)m_FileName,GENERIC_READ|GENERIC_WRITE,shareMode,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
        } else
        {
		    m_Handle = CreateFileA(CStr(m_FileName, m_FileName.GetHeap()).Get(),GENERIC_READ|GENERIC_WRITE,shareMode,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
        }
...
void CFile::OpenRead(DWORD shareMode)
{
	if(m_Open==0)
    {
        if (IS_UNICODE())
        {
		    m_Handle=CreateFileW(m_FileName.Get(),GENERIC_READ,shareMode,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
        } else
        {
...
void CFile::Create(DWORD shareMode)
{
	if(m_Open==0)
    {
        if (IS_UNICODE())
        {
		    m_Handle = CreateFileW(m_FileName.Get(),GENERIC_READ|GENERIC_WRITE,shareMode,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
        } else
        {
		    m_Handle = CreateFileA(CStr(m_FileName, m_FileName.GetHeap()).Get(),GENERIC_READ|GENERIC_WRITE,shareMode,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
        }
		if (m_Handle==INVALID_HANDLE_VALUE)
        {
