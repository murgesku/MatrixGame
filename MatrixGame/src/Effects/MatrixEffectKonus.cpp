...
CMatrixEffectKonus::CMatrixEffectKonus(const D3DXVECTOR3 &start, const D3DXVECTOR3 &dir, float radius, float height, float an, float ttl, bool intense, CTextureManaged *tex):
CMatrixEffect(), m_Pos(start), m_Dir(dir), m_Radius(radius), m_Height(height), m_TTL(ttl), m_Intense(intense), m_Tex(tex), m_Angle(an),
_m_TTL(INVERT(ttl))
{
    DTRACE();
    ...
    if (!m_Tex) m_Tex = (CTextureManaged *)g_Cache->Get(cc_TextureManaged,TEXTURE_PATH_GUN_FIRE);
    //m_Tex->Prepare();
    m_Tex->RefInc();
...
bool CMatrixEffectKonus::PrepareDX(void)
{
    ...
    v[KONUS_NUM_SECTORS+1].p = D3DXVECTOR3(1,1,0); v[KONUS_NUM_SECTORS+1].tu = 1; v[KONUS_NUM_SECTORS+1].tv = 0.5f;
...
void CMatrixEffectKonus::Draw(void)
{
    ...
    if (m_Intense) g_D3DD->SetRenderState(D3DRS_DESTBLEND,  D3DBLEND_INVSRCALPHA  );
}
...
void CMatrixEffectKonus::UpdateMatrix(void)
{
    ...
    m_Mat._11 = m_Height;   m_Mat._12 = 0;               m_Mat._13 = 0;           m_Mat._14 = 0;
    m_Mat._21 = 0;          m_Mat._22 = cc*m_Radius;     m_Mat._23 = ss*m_Radius; m_Mat._24 = 0;
    m_Mat._31 = 0;          m_Mat._32 = -ss*m_Radius;    m_Mat._33 = cc*m_Radius; m_Mat._34 = 0;
    m_Mat._41 = 0;          m_Mat._42 = 0;               m_Mat._43 = 0;           m_Mat._44 = 1;
