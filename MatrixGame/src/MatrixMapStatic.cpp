...
bool CMatrixMapStatic::RenderToTexture(SRenderTexture *rt, int n, /*float *fff,*/  float anglez, float anglex, float fov)
{
    DTRACE();
#define RENDSZ  256
    ...
    if (CustomZ)
    {
        ...
        oldZ->GetDesc(&d);
        if (D3D_OK != g_D3DD->CreateDepthStencilSurface(RENDSZ,RENDSZ, d.Format, d.MultiSampleType, d.MultiSampleQuality, TRUE, &newZ, NULL))
        {
            newTarget->Release();
            return NULL;
        }
...
*/
	...
	D3DXMatrixLookAtLH (&matView,
        &fc,
		&D3DXVECTOR3 (-right.x * 3, -right.y * 3, h),
...
void      CMatrixMapStatic::OnEndOfDraw(void)
{
    ...
    for (;f;)
    {
        ...
        if (f->GetObjectType() == OBJECT_TYPE_FLYER)
        {
            ((CMatrixFlyer *)f)->OnOutScreen();

        } else
        if (f->GetObjectType() == OBJECT_TYPE_ROBOTAI)
        {
            ((CMatrixRobotAI *)f)->OnOutScreen();

        } else
        if (f->GetObjectType() == OBJECT_TYPE_BUILDING)
        {
