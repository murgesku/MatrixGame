...
CMatrixEffectRepair::CMatrixEffectRepair(const D3DXVECTOR3 &pos, const D3DXVECTOR3 &dir, float seekradius, CMatrixMapStatic * skip):
CMatrixEffect(), m_BBCnt(0), m_Time(0), m_NextSpawnTime(0), m_SeekRadius(seekradius), m_Target(NULL), m_OffTargetAmp(0), m_Skip(skip),
m_Pos(pos), m_Dir(dir), m_NextSeekTime(0), m_ChangeTime(0),m_KordOnTarget(m_Heap),m_Kord(m_Heap)
{
    DTRACE();
...
void CMatrixEffectRepair::Takt(float t)
{
    ...
    float sins[10] = 
    {
        (float)sin(m_Time * 0.005f + 0.9f),
        (float)sin(m_Time * 0.007f + 0.2f),
        (float)sin(m_Time * 0.0073f + 0.3f),
        (float)sin(m_Time * 0.0013f),
        (float)sin(m_Time * 0.0022f + 0.2f),
        (float)sin(m_Time * 0.0017f + 0.8f),
        (float)sin(m_Time * 0.0028f + 0.3f),
        (float)sin(m_Time * 0.0051f + 0.9f),
        (float)sin(m_Time * 0.003f + 0.01f),
        (float)sin(m_Time * 0.001f + 0.93f)
    };
    ...
    while (m_NextSpawnTime < m_Time)
    {
        ...
        if (m_BBCnt < REPAIR_BB_CNT)
        {
            ...
            m_BBoards[m_BBCnt].bb.CBillboardLine::CBillboardLine(TRACE_PARAM_CALL p,p,4, 0x00FFFFFF, GetBBTexI(BBT_REPGLOWEND));
