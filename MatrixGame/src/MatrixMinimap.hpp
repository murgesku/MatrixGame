...
class CMinimap : public CMain
{
    ...
    bool IsOut(const D3DXVECTOR2 &pos) const {return (pos.x < float(m_PosX)) || (pos.x > float(m_PosX + m_SizeX)) || (pos.y < float(m_PosY)) || (pos.y > float(m_PosY + m_SizeY)); };

    void Clip(D3DXVECTOR2 &out, const D3DXVECTOR2& in);

public:
    CMinimap(void):m_Texture(NULL), m_Dirty(1),m_Time(0),m_TgtScale(1), m_Scale(1),m_TextureStore(NULL) {memset(m_Tex,0,sizeof(m_Tex));};
    ~CMinimap(void) {};
    ...
    void SetOutParams(int x, int y, int sx, int sy, const D3DXVECTOR2 &center, float scale, DWORD color)
    {
        ...
        m_Color = color;
        m_Center = center;
        if (scale < MINIMAP_MIN_SCALE) scale = MINIMAP_MIN_SCALE;
        if (scale > MINIMAP_MAX_SCALE) scale = MINIMAP_MAX_SCALE;
        m_TgtScale = scale;
        m_Dirty = 1;
    }
