...
//Ramka preparation
    ...
    while(els){
        if(els->m_strName == IF_POPUP_TOPLEFT){
            ...
            //CPoint sou_tp(Float2Int(els->m_xTexPos), Float2Int(els->m_yTexPos));
            //bm_src.CreateRGBA(Float2Int(els->m_TexWidth), Float2Int(els->m_TexHeight), lr_src.Pitch, lr_src.pBits);
            //bm_dest.MergeWithAlpha(CPoint(LEFT_SPACE+1,cursik_hpos), CPoint(Float2Int(els->m_Width), Float2Int(els->m_Height)), bm_src, sou_tp);
            //els->m_Image->UnlockRect();
        }
        els = els->m_NextImage;
    }
...
{
    if(type == MRT_WEAPON && kind == 0) return 0;
    if(type == MRT_HEAD && kind == 0) return 0;

    if(type == MRT_WEAPON && kind == 1) return 1;
    if(type == MRT_WEAPON && kind == 2) return 2;
    if(type == MRT_WEAPON && kind == 3) return 3;
    if(type == MRT_WEAPON && kind == 4) return 4;
    if(type == MRT_WEAPON && kind == 5) return 1;
    if(type == MRT_WEAPON && kind == 6) return 5;
    if(type == MRT_WEAPON && kind == 7) return 2;
    if(type == MRT_WEAPON && kind == 8) return 6;
    if(type == MRT_WEAPON && kind == 9) return 7;
    if(type == MRT_WEAPON && kind == 10) return 8;

    if(type == MRT_HEAD && kind == 1) return 1;
    if(type == MRT_HEAD && kind == 2) return 2;
    if(type == MRT_HEAD && kind == 3) return 3;
    if(type == MRT_HEAD && kind == 4) return 4;

    if(type == MRT_CHASSIS && kind == 1) return 0;
    if(type == MRT_CHASSIS && kind == 2) return 1;
    if(type == MRT_CHASSIS && kind == 3) return 2;
    if(type == MRT_CHASSIS && kind == 4) return 3;
    if(type == MRT_CHASSIS && kind == 5) return 4;

    if(type == MRT_ARMOR && kind == 1) return 1;
    if(type == MRT_ARMOR && kind == 2) return 2;
    if(type == MRT_ARMOR && kind == 3) return 3;
    if(type == MRT_ARMOR && kind == 4) return 4;
    if(type == MRT_ARMOR && kind == 5) return 5;
    if(type == MRT_ARMOR && kind == 6) return 0;
