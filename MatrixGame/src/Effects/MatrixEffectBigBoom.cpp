...
CMatrixEffectBigBoom::CMatrixEffectBigBoom(const D3DXVECTOR3 &pos, float radius, float ttl,DWORD hitmask, CMatrixMapStatic * skip, DWORD user, FIRE_END_HANDLER handler, DWORD light):
CMatrixEffect(), m_Radius(radius),m_TTL(ttl), _m_TTL(INVERT(ttl)), m_User(user), m_Handler(handler), m_hitmask(hitmask), m_skip(skip)
#ifdef _DEBUG
...
bool CMatrixEffectBigBoom::PrepareDX(void)
{
    ...
    m_Tris[0].i1 = 0; m_Tris[0].i2 = 1; m_Tris[0].i3 = 2;
    m_Tris[1].i1 = 0; m_Tris[1].i2 = 1; m_Tris[1].i3 = 3;
    m_Tris[2].i1 = 0; m_Tris[2].i2 = 2; m_Tris[2].i3 = 3;

    m_Tris[3].i1 = 4; m_Tris[3].i2 = 1; m_Tris[3].i3 = 2;
    m_Tris[4].i1 = 4; m_Tris[4].i2 = 1; m_Tris[4].i3 = 3;
    m_Tris[5].i1 = 4; m_Tris[5].i2 = 2; m_Tris[5].i3 = 3;
    ...
    while (pass-- >0)
    {
        ...
        {
            D3DXVECTOR3  dir( (m_Pts[m_Tris[i].i1].p.x + m_Pts[m_Tris[i].i2].p.x + m_Pts[m_Tris[i].i3].p.x) * (1.0f/3.0f), 
...
CMatrixEffectBigBoom::~CMatrixEffectBigBoom()
{
    ...
    if (m_Handler) m_Handler(TRACE_STOP_NONE, D3DXVECTOR3(m_Mat._41,m_Mat._42,WATER_LEVEL), m_User, FEHF_LASTHIT);
}
...
{
    DTRACE();
    if (FLAG(m_Flags, BIGBOOMF_PREPARED)) return;
    m_Tex->Preload();
...
void CMatrixEffectBigBoom::Takt(float step)
{
    ...
    if (m_TTL < 0)
    {
        if (m_Handler)
        {
            ...
            {
                g_MatrixMap->FindObjects(*(D3DXVECTOR3 *)&m_Mat._41, m_Radius, 1, m_hitmask, m_skip, BoomEnum, (DWORD)this);
            }
    ...
    if (m_Handler)
    {
        if (m_hitmask & TRACE_ANYOBJECT)
        {
            g_MatrixMap->FindObjects(*(D3DXVECTOR3 *)&m_Mat._41, m_Radius, 1, m_hitmask, m_skip, BoomEnumNaklon, (DWORD)this);
        }
    }
    ...
    int a = Float2Int (k * 512); if (a > 255) a = 255;
