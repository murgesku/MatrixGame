...
CMatrixEffectBillboard::CMatrixEffectBillboard(const D3DXVECTOR3 &pos, float radius1,float radius2, DWORD c1, DWORD c2, float ttl, float delay, const wchar *tex, const D3DXVECTOR3 &dir, ADD_TAKT addtakt):
CMatrixEffect(), m_Radius1(radius1),  m_Radius2(radius2), m_Color1(c1), m_Color2(c2), m_TTL(ttl),m_TTL2(ttl), _m_TTL(INVERT(ttl)), m_Delay(delay), m_Dir(dir), m_AddTaktHandler(addtakt)
{
    DTRACE();
...
void CMatrixEffectBillboard::Takt(float step)
{
    ...
    if (m_AddTaktHandler) { m_AddTaktHandler(this, step); }
...
CMatrixEffectBillboardLine::CMatrixEffectBillboardLine(const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1,float width, DWORD c1, DWORD c2, float ttl, CTextureManaged *tex):
