...
CWStr::CWStr(const CStr & s):CMain()
    { NewDataLen(s.GetHeap(), s.Len()); Set(s); }
...
{
    ++call_num;
	if(cstr.Len()<=0) { ModifyLen(m_Data->m_Heap, 0); return; }
...
double CWStr::GetDouble() const 
{
	...
	for(i=0;i<tlen;i++) if(tstr[i]=='-') { zn=-zn; break; }
...
bool CWStr::IsOnlyInt() const
{
	...
	for(int i=0;i<tlen;i++) if(tstr[i]<L'0' || tstr[i]>L'9' && tstr[i]!=L'-') return 0;
	return 1;
}
...
CWStr & CWStr::Trim()
{
	...
	if(i==tlen) { ModifyLenNoCopy(GetHeap(), 0); return *this; }
    int u;
	for(u=tlen-1;u>=0;u--)
    {
	...
	tlen=u-i+1;
	if(tlen<1) { ModifyLenNoCopy(GetHeap(), 0); return *this; }
...
int CWStr::FindR(wchar ch,int sme) const
{
	...
	while(sme>=0 && *curch!=ch) { sme--; curch--; }
	return sme;
}
...
int CWStr::GetLenPar(int smepar,const wchar * ogsim) const
{
	...
	int tlen=GetLen(); 
	int lenogsim=WStrLen(ogsim);
	if(tlen<1 || lenogsim<1 || smepar>tlen) ERROR_E;
...
{
	int lens1=GetLen(); if(lens1<1) return false;
	int lens2=str.GetLen(); if(lens2<1) return false;
