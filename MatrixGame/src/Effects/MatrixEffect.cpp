...
static const SBillboardID2Texname bb2tn[] = 
{
    {BBT_SMOKE,         BB_TEXTURE_NAME_SMOKE},
...
void    CEffectSpawner::Takt(float)
{
    if (m_Under == NULL)
    {
        ...
        g_MatrixMap->FindObjects(m_Props->m_Pos, 100, 1.0f, TRACE_BUILDING|TRACE_OBJECT,NULL, FindOwner, (DWORD)&data);
        if (m_Under == NULL)
        {
...
bool CEffectSpawner::OutOfTime(void) const {return (m_TTL > 1) && (g_MatrixMap->GetTime() > m_TTL);}


void EffectSpawnerFire(SpawnEffectProps *props)
{
    SpawnEffectFire *fire = (SpawnEffectFire *)props;
    CMatrixEffect::CreateFire(NULL, fire->m_Pos, fire->m_ttl, fire->m_puffttl, fire->m_spawntime, fire->m_dispfactor, fire->m_intense, fire->m_speed);
}
void EffectSpawnerSmoke(SpawnEffectProps *props)
{
    SpawnEffectSmoke *smoke = (SpawnEffectSmoke *)props;
    CMatrixEffect::CreateSmoke(NULL, smoke->m_Pos, smoke->m_ttl, smoke->m_puffttl, smoke->m_spawntime, smoke->m_color, smoke->m_intense, smoke->m_speed);
}
...
void    CMatrixEffect::ClearEffects(void)
{
    ...
    if (m_Debris) HFree(m_Debris, m_Heap);
    m_DebrisCnt = 0;
    m_Debris = 0;
...
void CMatrixEffect::InitEffects(CBlockPar &bp_in)
{
    ...
    m_SpotProperties[SPOT_SOLE_TRACK].texture = (CTextureManaged *)g_Cache->Get(cc_TextureManaged,TEXTURE_PATH_SOLE_TRACK);
    ...
    m_SpotProperties[SPOT_SOLE_WHEEL].texture = (CTextureManaged *)g_Cache->Get(cc_TextureManaged,TEXTURE_PATH_SOLE_WHEEL);
    ...
    m_SpotProperties[SPOT_SOLE_PNEUMATIC].texture = (CTextureManaged *)g_Cache->Get(cc_TextureManaged,TEXTURE_PATH_SOLE_PNEUMATIC);
    ...
    //CTextureManaged *ti = (CTextureManaged *)g_Cache->Get(cc_TextureManaged, bb.ParGet(PAR_SOURCE_BILLBOARDS_TEXINTENSE));
    ts->Preload();
    //ti->Preload();
...
void CMatrixEffect::CreateExplosion(const D3DXVECTOR3 &pos, const SExplosionProperties &props, bool fire)
{
    ...
    if (props.voronka != SPOT_TYPES_CNT)
    {
        ...
        if (g && !g->IsBaseOn())
        {
            CMatrixEffect::CreateLandscapeSpot(NULL, D3DXVECTOR2(pos.x, pos.y), FSRND(M_PI), props.voronka_scale, props.voronka);
        }
    }
}

void CMatrixEffect::CreateSmoke(SEffectHandler *eh, const D3DXVECTOR3 &pos, float ttl, float puffttl, float spawntime, DWORD color, bool intense, float speed)
{
    DTRACE();
...
void CMatrixEffect::CreateFire(SEffectHandler *eh, const D3DXVECTOR3 &pos, float ttl, float puffttl, float spawntime, float dispfactor, bool intense, float speed)
{
    DTRACE();
...
#include "stdio.h"
#endif
void  CMatrixEffect::CreateLandscapeSpot(SEffectHandler *eh, const D3DXVECTOR2 &pos, float angle, float scale, ESpotType  type)
{
    DTRACE();
...
void CMatrixEffect::CreateKonus(SEffectHandler *eh, const D3DXVECTOR3 &start, const D3DXVECTOR3 &dir, float radius, float height, float angle, float ttl, bool intense, CTextureManaged *tex)
{
    DTRACE();
...
void CMatrixEffect::CreateKonusSplash(const D3DXVECTOR3 &start, const D3DXVECTOR3 &dir, float radius, float height, float angle, float ttl, bool intense, CTextureManaged *tex)
{
    DTRACE();

    if (MAX_EFFECT_DISTANCE_SQ < D3DXVec3LengthSq(&(start-g_MatrixMap->m_Camera.GetFrustumCenter()))) return;

    CMatrixEffectKonusSplash *e = HNew(m_Heap) CMatrixEffectKonusSplash(start, dir, radius, height, angle, ttl, intense, tex);
...
CMatrixEffect * CMatrixEffect::CreateWeapon(const D3DXVECTOR3 &start, const D3DXVECTOR3 &dir, DWORD user, FIRE_END_HANDLER handler, EWeapon type, int cooldown)
{
    DTRACE();
...
void CMatrixEffect::CreateFlame(SEffectHandler *eh, float ttl, DWORD hitmask, CMatrixMapStatic * skip, DWORD user, FIRE_END_HANDLER handler)
{
    DTRACE();
...
void CMatrixEffect::CreateBigBoom(const D3DXVECTOR3 &pos, float radius, float ttl,DWORD hitmask, CMatrixMapStatic * skip, DWORD user, FIRE_END_HANDLER handler, DWORD light)
{
    DTRACE();
...
void CMatrixEffect::CreateLightening(SEffectHandler *eh, const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1,float ttl,float dispers, float width, DWORD color)
{
    DTRACE();
...
void CMatrixEffect::CreateBillboard(SEffectHandler *eh, const D3DXVECTOR3 &pos, float radius1,float radius2, DWORD c1, DWORD c2, float ttl, float delay,const wchar *tex, const D3DXVECTOR3 &dir, ADD_TAKT addtakt)
{
    DTRACE();
    CMatrixEffectBillboard *e = HNew(m_Heap) CMatrixEffectBillboard(pos, radius1, radius2,c1,c2,ttl,delay,tex,dir,addtakt);
    if (!g_MatrixMap->AddEffect(e)) e = NULL;
    if (eh && e)
    {
#ifdef _DEBUG
...
void CMatrixEffect::CreateBillboardLine(SEffectHandler *eh, const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1,float width, DWORD c1, DWORD c2, float ttl, CTextureManaged *tex)
{
    DTRACE();
...
CMatrixEffect * CMatrixEffect::CreateRepair(const D3DXVECTOR3 &pos,const D3DXVECTOR3 &dir, float seekradius, CMatrixMapStatic * skip)
{
    DTRACE();
