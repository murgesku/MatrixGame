...
void CMatrixMapLogic::Clear()
{
	...
	m_MPFCnt=m_MPF2Cnt=0;
	if(m_MPF) { HFree(m_MPF,g_MatrixHeap); m_MPF=NULL; }
	if(m_MPF2) { HFree(m_MPF2,g_MatrixHeap); m_MPF2=NULL; }

    if(m_ZoneIndex) { HFree(m_ZoneIndex,g_MatrixHeap); m_ZoneIndex=NULL; }
    if(m_ZoneIndexAccess) { HFree(m_ZoneIndexAccess,g_MatrixHeap); m_ZoneIndexAccess=NULL; }
    if(m_ZoneDataZero) { HFree(m_ZoneDataZero,g_MatrixHeap); m_ZoneDataZero=NULL; }

    if(m_MapPoint) { HFree(m_MapPoint,g_MatrixHeap); m_MapPoint=NULL; }
...
//        if (sme >= maxmax || cnt >=maxmax) break;
        ...
        for(u=0;u<4;u++) {
            tp=m_MapPoint[sme];
            switch(u) {
                case 0: tp.x--; if(tp.x<re.left) continue; break;
                case 1: tp.y--; if(tp.y<re.top) continue; break;
                case 2: tp.x++; if(tp.x>=re.right) continue; break;
                case 3: tp.y++; if(tp.y>=re.bottom) continue; break;
            }
            ...
            if(!findok && tp.x==dx && tp.y==dy) { tpfind.x=dx; tpfind.y=dy; findok=1; }
...
//#endif
    ...
    while(smm->m_Find) {
        ...
        if(MatrixPathMoveMax-1-cnt<=0) ERROR_E;
        path[MatrixPathMoveMax-1-cnt]=tpfind;
        cnt++;
    }
...
if(test && !g_TestLocal) CHelper::DestroyByGroup(100);
    ...
    if(route) { // Устанавливаем доступ
        for(int i=1;i<route->m_Header[routeno].m_Cnt;i++) {
            ...
            for(int u=0;u<road->m_ZoneCnt;u++) {
                ...
                if(!z1.m_Access) {
                z1.m_Access=true;
                m_ZoneIndexAccess[accesscnt]=road->m_Zone[u]; accesscnt++;
                }
                ...
                for(int t=0;t<z1.m_NearZoneCnt;t++) {
                    ...
                    if(!z2.m_Access) {
                    z2.m_Access=true;
                    m_ZoneIndexAccess[accesscnt]=z1.m_NearZone[t]; accesscnt++;
                    }
                    ...
                    for(int k=0;k<z2.m_NearZoneCnt;k++) {
                        ...
                        if(!z3.m_Access) {
                        z3.m_Access=true;
                        m_ZoneIndexAccess[accesscnt]=z2.m_NearZone[k]; accesscnt++;
                    }
                }
            }
        }
    ...
    if(zonefindok<0) { // Не нашли ни какого пути
        ...
        while(curzone!=zend) {
            ...
            if(i>=zone->m_NearZoneCnt) ERROR_E;
        }
        ...
        while(curzone!=zend) {
            ...
            for(int i=0;i<zone->m_NearZoneCnt;i++) {
                ...
                if(zone2->m_FPLevel<level) {
                    ...
                    if(!zone2->m_Access) {
                    zone2->m_Access=true;
                    m_ZoneIndexAccess[accesscnt]=newzone; accesscnt++;
                    }
                    zone=m_RN.m_Zone+curzone;
                    for(u=0;u<zone->m_NearZoneCnt;u++) {
                        ...
                        if(!zone2->m_Access) {
                        zone2->m_Access=true;
                        m_ZoneIndexAccess[accesscnt]=zone->m_NearZone[u]; accesscnt++;
                    }
                    }
                    ok=true;
                    break;
            ...
            if(!ok/*i>=zone->m_NearZoneCnt*/) ERROR_E;
        }
    }
...
/*			if(size==1) {
				...
				smmt=smm;
				for(int y=0;y<size;y++,smmt+=m_SizeMove.x-size) for(int x=0;x<size;x++,smmt++) if(smmt->m_Stop & (1<<nsh)) return false;
			}*/
		}
	} else {
		int d=(dx<<1) - dy;
...
int CMatrixMapLogic::RandomizeMovePath(int nsh,int size,int cnt,CPoint * path)
{
	...
	for(int i=1;i<(cnt-1);i++) {
		...
		for(int u=0;u<5;u++) {
			...
			if(!PlaceFindNear(nsh,size,newpx,newpy,0,NULL,NULL)) continue;
    ...
    while(sme<cnt) {
        if(m_RN.m_Zone[m_ZoneIndex[sme]].m_PlaceCnt>0) {
            ...
            for(i=0;i<m_RN.m_Zone[m_ZoneIndex[sme]].m_NearZoneCnt;i++) {
                ...
                if(m_RN.m_Zone[m_ZoneIndex[sme]].m_NearZoneMove[i] & mm) continue;
...
int CMatrixMapLogic::PlaceList(byte mm,CPoint & from,CPoint & to,int radius,bool farpath,int * list,int * listcnt,int * outdist)
{
    ...
    *listcnt=0;
    ...
    if(toplace>=0 && ((m_RN.GetPlace(toplace)->m_Move & (1+2+4+8+16))==(1+2+4+8+16) || !(m_RN.GetPlace(toplace)->m_Move & mm))) {
        if(*listcnt>=m_RN.m_PlaceCnt) __asm int 3;
        ...
        while(sme<cnt) {
            ...
            for(i=0;i<place->m_NearCnt;i++) {
                ...
                if(to.Dist2(m_RN.m_Place[np].m_Pos)>POW2(radius)) continue;
    ...
    *listcnt = 0;
    ...
    while(sme<cnt) {
        if(m_ZoneIndex[sme]==fromplace) { u=m_ZoneDataZero[m_ZoneIndex[sme]] & 0x7fff0000; break; }
        place=m_RN.GetPlace(m_ZoneIndex[sme]);
        for(i=0;i<place->m_NearCnt;i++) {
            np=place->m_Near[i];
            if(m_ZoneDataZero[np] & 0x7fff0000) continue;
            ...
            if(np==fromplace) { u=m_ZoneDataZero[m_ZoneIndex[sme]] & 0x7fff0000; break; }
...
static bool Egg1(const D3DXVECTOR2 & center, CMatrixMapStatic *ms, DWORD user)
{
    ...
    if (ms->IsLiveRobot())
    {
        ...
        if ((!ms->AsRobot()->HaveBomb() || fabs(ms->AsRobot()->GetMaxHitPoint() - ms->AsRobot()->GetHitPoint()) > 1) && !ms->AsRobot()->IsInPosition())
        {
            (*egg) += 100;
        }
...
void CMatrixMapLogic::DumpLogic()
{
    ...
    char * rss[]={"ROBOT_IN_SPAWN","ROBOT_BASE_MOVEOUT","ROBOT_SUCCESSFULLY_BUILD","ROBOT_CARRYING","ROBOT_FALLING","ROBOT_DIP","ROBOT_BASE_CAPTURE","ROBOT_EMBRYO"};
    char * ros[]={"ROBOT_EMPTY_ORDER","ROT_MOVE_TO","ROT_MOVE_BACK","ROT_MOVE_RETURN","ROT_STOP_MOVE","ROT_FIRE","ROT_STOP_FIRE","ROT_CAPTURE_FACTORY","ROT_STOP_CAPTURE"};
    ...
    if((fi=fopen("#DumpLogic.txt","w+"))==NULL) return;
            ...
            for(int ig=0;ig<MAX_LOGIC_GROUP;ig++) {
                ...
                while(ms) {
                    if(ms->IsRobot() && ms->AsRobot()->m_Side == m_Side[is].m_Id && ms->AsRobot()->GetGroupLogic()==ig) {
                        ...
                        else fprintf(fi,"            m_Environment.m_Place=%d    Region=%d\n",env->m_Place,g_MatrixMap->m_RN.GetPlace(env->m_Place)->m_Region);
