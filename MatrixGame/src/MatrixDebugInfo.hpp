...
public:
    ...
    void InitFont(void);
    void ClearFont(void) {if (m_Font) { m_Font->Release(); m_Font = NULL;}};

    void OnLostDevice(void) {if (m_Font) m_Font->OnLostDevice();}
    void OnResetDevice(void) {if (m_Font) m_Font->OnResetDevice();}
};
