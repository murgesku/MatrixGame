...
//#endif
        ...
        si = FindSlotForSound();
        m_AllSounds[si].id_internal = snd_create(m_Sounds[snd].Path().GetBuf(), m_LastGroup++, FLAG(m_Sounds[snd].flags, SSoundItem::LOOPED));
...
void CSound::AddSound(const wchar *name, const D3DXVECTOR3 &pos)
{
    ...
    AddSound(pos, m_Sounds[S_SPECIAL_SLOT].attn,
                      ...
                      m_Sounds[S_SPECIAL_SLOT].Path().GetBuf());
